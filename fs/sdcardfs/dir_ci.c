	...
	struct qstr *name, bool locked)
{
	...
	file = dentry_open(dir, O_RDONLY | O_DIRECTORY
		| O_NOATIME, current_cred()); /*lint !e666*/
...
sdcardfs_lookup_ci_with_find_entry(
	struct path *dir,
	struct qstr *orig, bool locked)
{
	int err;
	...
	err = parent->d_sb->s_op->find_entry_ci(
		d_inode(parent), orig, name);
	dentry = (err ? ERR_PTR(err) : lookup_one_len(name, parent, orig->len));
...
static int sdcardfs_may_create_ci_with_find_entry(
	struct path *dir, struct qstr *name)
{
	struct dentry *parent = dir->dentry;
	int ret = parent->d_sb->s_op->find_entry_ci(d_inode(parent), name, NULL);
	if (ret == -ENOENT)
		ret = 0;
	else if (!ret)
		ret = -EEXIST;
...
struct sdcardfs_dir_ci_ops sdcardfs_generic_dir_ci_op = {
	...
	.lookup = NULL,
	.may_create = NULL
}, sdcardfs_find_entry_dir_ci_op = {
	.lookup = sdcardfs_lookup_ci_with_find_entry,
	.may_create = sdcardfs_may_create_ci_with_find_entry
};
...
struct sdcardfs_dir_ci_ops *sdcardfs_lowerfs_ci_ops(struct super_block *sb)
{
	...
	/* If the underlayfs has a customized case-insensitive find_entry,
	   use it rather than iterate_dir */
	else if (sb->s_op->find_entry_ci != NULL)
		return &sdcardfs_find_entry_dir_ci_op;
