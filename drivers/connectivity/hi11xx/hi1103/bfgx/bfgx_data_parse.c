...
#ifdef CONFIG_HI1102_PLAT_HW_CHR
    CHR_ERRNO_WITH_ARG_STRU *pst_chr_errno = NULL;
#endif
    uint8 syschar;
    uint64 flags;
...
#ifdef CONFIG_HI1102_PLAT_HW_CHR
        pst_chr_errno = (CHR_ERRNO_WITH_ARG_STRU *)(buf_ptr+1);
        PS_PRINT_DBG("host CHR system get dev errno at bfgn, errno is 0x%x\n", pst_chr_errno->errno);
        __chr_exception(pst_chr_errno->errno);
#else
