...
OAL_STATIC wlan_cfg_cmd g_ast_wifi_config_cmds[] =
{
    ...
    {"ampdu_amsdu_skb",                 WLAN_CFG_INIT_AMPDU_AMSDU_SKB},
#endif
    /* 低功耗 */
...
OAL_STATIC wlan_cfg_cmd g_ast_nvram_config_ini[NVRAM_PARAMS_PWR_INDEX_BUTT] =
{
    ...
    {"fcc_txpwr_limit_5g_band1",  NVRAM_PARAMS_INDEX_23},
    {"fcc_txpwr_limit_5g_band2",  NVRAM_PARAMS_INDEX_24},
    {"fcc_txpwr_limit_5g_band3",  NVRAM_PARAMS_INDEX_25},
    {"fcc_txpwr_limit_5g_band4",  NVRAM_PARAMS_INDEX_26},
    {"fcc_txpwr_limit_5g_band5",  NVRAM_PARAMS_INDEX_27},
    {"fcc_txpwr_limit_5g_band6",  NVRAM_PARAMS_INDEX_28},
    {"fcc_txpwr_limit_5g_band7",  NVRAM_PARAMS_INDEX_29},
...
OAL_STATIC oal_void original_value_for_nvram_params(oal_void)
{
    ...
    /* FCC功率认证 */
    g_al_nvram_init_params[NVRAM_PARAMS_INDEX_23] = 0xFFFFFFFF;
    g_al_nvram_init_params[NVRAM_PARAMS_INDEX_24] = 0xFFFFFFFF;
...
OAL_STATIC oal_void host_params_init_first(oal_void)
{
    ...
    /* 2G RF前端 */
#if (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1103_HOST)
    g_al_host_init_params[WLAN_CFG_INIT_RF_LINE_TXRX_GAIN_DB_2G_BAND1_MULT4]    = 0xF4F4;
    g_al_host_init_params[WLAN_CFG_INIT_RF_LINE_TXRX_GAIN_DB_2G_BAND2_MULT4]    = 0xF4F4;
    g_al_host_init_params[WLAN_CFG_INIT_RF_LINE_TXRX_GAIN_DB_2G_BAND3_MULT4]    = 0xF4F4;
    /* 5G RF前端 */
    g_al_host_init_params[WLAN_CFG_INIT_RF_LINE_TXRX_GAIN_DB_5G_BAND1_MULT4]    = 0xF8F8;
    g_al_host_init_params[WLAN_CFG_INIT_RF_LINE_TXRX_GAIN_DB_5G_BAND2_MULT4]    = 0xF8F8;
    g_al_host_init_params[WLAN_CFG_INIT_RF_LINE_TXRX_GAIN_DB_5G_BAND3_MULT4]    = 0xF8F8;
    g_al_host_init_params[WLAN_CFG_INIT_RF_LINE_TXRX_GAIN_DB_5G_BAND4_MULT4]    = 0xF8F8;
    g_al_host_init_params[WLAN_CFG_INIT_RF_LINE_TXRX_GAIN_DB_5G_BAND5_MULT4]    = 0xF8F8;
    g_al_host_init_params[WLAN_CFG_INIT_RF_LINE_TXRX_GAIN_DB_5G_BAND6_MULT4]    = 0xF8F8;
    g_al_host_init_params[WLAN_CFG_INIT_RF_LINE_TXRX_GAIN_DB_5G_BAND7_MULT4]    = 0xF8F8;
...
OAL_STATIC oal_int32 hwifi_custom_adapt_device_priv_ini_radio_cap_param(oal_uint8 *puc_data, oal_uint32 *pul_data_len)
{
    ...
    st_syn_msg.ul_len = OAL_SIZEOF(g_auc_wlan_service_device_per_chip);

    oal_memcopy(puc_data, &st_syn_msg, CUSTOM_MSG_DATA_HDR_LEN);
    oal_memcopy(puc_data + CUSTOM_MSG_DATA_HDR_LEN, g_auc_wlan_service_device_per_chip, OAL_SIZEOF(g_auc_wlan_service_device_per_chip));

    *pul_data_len += (OAL_SIZEOF(g_auc_wlan_service_device_per_chip) + CUSTOM_MSG_DATA_HDR_LEN);

    OAM_WARNING_LOG2(0, OAM_SF_CFG, "{hwifi_custom_adapt_device_priv_ini_radio_cap_param::da_len[%d] radio_cap_0[%d].}",
                        *pul_data_len, g_auc_wlan_service_device_per_chip[0]);
...
OAL_STATIC oal_int32 hwifi_custom_adapt_mac_device_priv_ini_param(wlan_cfg_priv_id_uint8 uc_cfg_id, oal_uint8 *puc_data, oal_uint32 *pul_len)
{
    oal_int32                              l_ret;
    hmac_to_dmac_cfg_custom_data_stru      st_syn_msg;
    oal_int32                              l_priv_val   = 0 ;
    ...
    *pul_len += (OAL_SIZEOF(uc_priv_cfg_value) + CUSTOM_MSG_DATA_HDR_LEN);
    ...
    OAM_WARNING_LOG3(0, OAM_SF_CFG, "{priv_ini_param_cfg::cfg_id[%d] data_len[%d] cfg_val[0x%x].}", uc_cfg_id, *pul_len, uc_priv_cfg_value);
...
int32 hwifi_custom_adapt_device_priv_ini_param(oal_uint8 *puc_data)
{
    ...
    hwifi_custom_adapt_mac_device_priv_ini_param(WLAN_CFG_PRIV_BW_MAX_WITH, puc_data + ul_data_len, &ul_data_len);
    hwifi_custom_adapt_mac_device_priv_ini_param(WLAN_CFG_PRIV_LDPC_CODING, puc_data + ul_data_len, &ul_data_len);
    hwifi_custom_adapt_mac_device_priv_ini_param(WLAN_CFG_PRIV_RX_STBC, puc_data + ul_data_len, &ul_data_len);
    hwifi_custom_adapt_mac_device_priv_ini_param(WLAN_CFG_PRIV_TX_STBC, puc_data + ul_data_len, &ul_data_len);
    hwifi_custom_adapt_mac_device_priv_ini_param(WLAN_CFG_PRIV_SU_BFER, puc_data + ul_data_len, &ul_data_len);
    hwifi_custom_adapt_mac_device_priv_ini_param(WLAN_CFG_PRIV_SU_BFEE, puc_data + ul_data_len, &ul_data_len);
    hwifi_custom_adapt_mac_device_priv_ini_param(WLAN_CFG_PRIV_MU_BFER, puc_data + ul_data_len, &ul_data_len);
    hwifi_custom_adapt_mac_device_priv_ini_param(WLAN_CFG_PRIV_MU_BFEE, puc_data + ul_data_len, &ul_data_len);
    hwifi_custom_adapt_device_priv_ini_cali_mask_param(puc_data + ul_data_len, &ul_data_len);
    hwifi_custom_adapt_mac_device_priv_ini_param(WLAN_CFG_PRIV_CALI_DATA_MASK, puc_data + ul_data_len, &ul_data_len);
    hwifi_custom_adapt_mac_device_priv_ini_param(WLAN_CFG_PRIV_CALI_AUTOCALI_MASK, puc_data + ul_data_len, &ul_data_len);
#ifdef _PRE_WLAN_DOWNLOAD_PM
    hwifi_custom_adapt_device_priv_ini_download_pm_param(puc_data + ul_data_len, &ul_data_len);
#endif
#ifdef _PRE_WLAN_FEATURE_TXOPPS
    hwifi_custom_adapt_mac_device_priv_ini_param(WLAN_CFG_PRIV_TXOPPS_SWITCH, puc_data + ul_data_len, &ul_data_len);
#endif
...
oal_int32 hwifi_custom_host_read_dyn_cali_nvram(oal_void)
{
    ...
    oal_int8      *pc_ctx;
    oal_int8      *pc_token;
    oal_uint8      auc_cust_nvram_tmp[DY_CALI_PARAMS_LEN];
    oal_uint8      auc_cust_nvram_info[WLAN_CFG_DTS_NVRAM_RATIO_PA5GA0_BAND1][DY_CALI_PARAMS_LEN];  /* NVRAM数组 */
    ...
    for (uc_idx = WLAN_CFG_DTS_NVRAM_RATIO_PA2GCCKA0; uc_idx <= WLAN_CFG_DTS_NVRAM_RATIO_PA5GA1; uc_idx++)
    {
        ...
        while (uc_times_idx)
        {
            if (0 == s_params[(uc_times_idx-1)*DY_CALI_PARAMS_TIMES])
            {
                OAM_ERROR_LOG1(0, OAM_SF_CUSTOM, "hwifi_custom_host_read_dyn_cali_nvram::check NV id[%d]!",
                               g_ast_wifi_nvram_cfg_handler[uc_idx].ul_nv_map_idx);
                OAL_MEMZERO(auc_cust_nvram_info[uc_idx], DY_CALI_PARAMS_LEN*OAL_SIZEOF(oal_uint8));
...
OAL_STATIC int32 hwifi_config_init_fcc_txpwr_nvram(void)
{
    ...
    int32     al_nvram_params[NVRAM_PARAMS_INDEX_43 - NVRAM_PARAMS_INDEX_23] = {0};

    for (uc_cfg_id = NVRAM_PARAMS_INDEX_23; uc_cfg_id < NVRAM_PARAMS_INDEX_43; uc_cfg_id++)
    {
    ...
    /* 2.4g */
    oal_memcopy(g_st_cust_nv_params.auc_5g_fcc_txpwr_limit_params, al_nvram_params,
             OAL_SIZEOF(g_st_cust_nv_params.auc_5g_fcc_txpwr_limit_params));
    /* 5g */
    for (uc_cfg_id = 0; uc_cfg_id < MAC_2G_CHANNEL_NUM; uc_cfg_id++)
    {
        oal_memcopy(g_st_cust_nv_params.auc_2g_fcc_txpwr_limit_params[uc_cfg_id], al_nvram_params + MAC_NUM_5G_BAND + uc_cfg_id,
                    CUS_NUM_FCC_2G_PRO);
    }
...
OAL_STATIC oal_uint32 hwifi_config_init_dy_cali_custom(oal_void)
{
    ...
    oal_int8             *pc_sep = ",";
    oal_uint8             uc_idx = 0;
    oal_uint16            ul_param_num = 0;
    oal_uint8             uc_rf_idx;
    oal_uint8             uc_cali_param_idx;
    oal_int16             s_params[WLAN_RF_CHANNEL_NUMS*DY_CALI_PARAMS_NUM*DY_CALI_PARAMS_TIMES] = {0};
    ...
    {
        if (WLAN_CFG_DTS_NVRAM_RATIO_PA5GA0_BAND1 > ul_cfg_id)
        {
            ...
            else
            {
                ...
                {
                    OAM_WARNING_LOG1(0, OAM_SF_CUSTOM, "hwifi_config_init_dy_cali_custom read, check id[%d] exists!", ul_cfg_id);
                    ul_ret = OAL_FAIL;
                    break;
                }
                /* 提供产线第一次上电校准初始值 */
        ...
        if (OAL_PTR_NULL == pc_token)
        {
            OAM_ERROR_LOG1(0, OAM_SF_CUSTOM, "hwifi_config_init_dy_cali_custom read get null value, check id[%d] exists!", ul_cfg_id);
            ul_ret =  OAL_FAIL;
            break;
        }
        pc_token = oal_strtok(pc_token, pc_sep, &pc_ctx);
        ...
        while (OAL_PTR_NULL != pc_token)
        {
            OAL_IO_PRINT( "hwifi_config_init_dy_cali_custom get [%s]\n!", pc_token);
            *(s_params + ul_param_num) = (oal_int16)oal_strtol(pc_token, OAL_PTR_NULL, 10);
            pc_token = oal_strtok(OAL_PTR_NULL, pc_sep, &pc_ctx);
            ul_param_num++;
        }
    }
    ...
    {
        if (ul_param_num != OAL_SIZEOF(s_params)/OAL_SIZEOF(oal_int16))
        {
            OAM_ERROR_LOG1(0, OAM_SF_CUSTOM, "hwifi_config_init_dy_cali_custom read get wrong ini value num[%d]!", ul_param_num);
            OAL_MEMZERO(g_as_pro_line_params, OAL_SIZEOF(g_as_pro_line_params));
            return OAL_FAIL;
        }
        for (uc_rf_idx = 0; uc_rf_idx < WLAN_RF_CHANNEL_NUMS; uc_rf_idx++)
        {
            for (uc_cali_param_idx = 0; uc_cali_param_idx < DY_CALI_PARAMS_NUM; uc_cali_param_idx++)
            {
...
int hwifi_get_cfg_params(void)
{
    ...
    for (l_cfg_id = 0; l_cfg_id < MAC_NUM_5G_BAND; l_cfg_id++)
    {
        for (l_cfg_band = 0; l_cfg_band < CUS_NUM_5G_BW; l_cfg_band++)
        {
            OAL_IO_PRINT("%s[%d] [%d] \t [config:%d]\n", "fcc_txpwr_limit_5g: band",l_cfg_id, l_cfg_band,
                          g_st_cust_nv_params.auc_5g_fcc_txpwr_limit_params[l_cfg_id][l_cfg_band]);
        }
    }
