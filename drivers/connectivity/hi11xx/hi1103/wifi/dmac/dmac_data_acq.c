...
oal_uint32  dmac_data_acq_prepare(mac_vap_stru *pst_mac_vap,oal_uint8 *puc_param, mac_device_stru *pst_device)
{
    ...
    /* 设置MAC寄存器 */
    hal_set_daq_mac_reg(pst_hal_device,
...
oal_uint32  dmac_data_acq_condition(mac_vap_stru *pst_mac_vap,oal_uint8 *puc_param)
{
    oal_uint32                  ul_offset = 0;
    oal_int8                    ac_name[DMAC_HIPRIV_CMD_NAME_MAX_LEN];
    oal_uint8                   uc_value;
    ...
    if (OAL_PTR_NULL == pst_hal_device)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{dmac_data_acq_condition::vap id [%d],DMAC_VAP_GET_HAL_DEVICE null}",pst_mac_vap->uc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (WLAN_RF_CHANNEL_NUMS < uc_value)
    {
    ...
    if (((oal_uint8)DMAC_DATA_ACQ_BIT_BUTT) <= uc_value)
    {
    ...
    /* 设置PHY寄存器 */
    hal_set_daq_phy_reg(pst_hal_device, ul_val);
...
oal_uint32  dmac_data_acq_enable(mac_vap_stru *pst_mac_vap,oal_uint8 *puc_param, mac_device_stru *pst_device)
{
    ...
    if (OAL_PTR_NULL == pst_hal_device)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{dmac_data_acq_enable::vap id [%d],DMAC_VAP_GET_HAL_DEVICE null}",pst_mac_vap->uc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    /* 设置MAC与PHY寄存器 */
    hal_set_daq_en(pst_hal_device, (oal_uint8)(g_st_data_acq.st_daq_prep_info.en_daq_enable));
...
oal_uint32  dmac_data_acq_get_status(mac_vap_stru *pst_mac_vap,oal_uint8 *puc_param, mac_device_stru *pst_device)
{
    ...
    if (OAL_PTR_NULL == pst_hal_device)
    {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{dmac_data_acq_get_status::vap id [%d],DMAC_VAP_GET_HAL_DEVICE null}",pst_mac_vap->uc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }

    /* 回读寄存器 */
    hal_get_daq_status(pst_hal_device, &ul_reg_value);
...
oal_uint32  dmac_data_acq_get_record(oal_void)
{
    ...
    if (DMAC_DATA_ACQ_STATUS_COMPLETE == g_st_data_acq.en_daq_status)
    {
        ...
        /* 无效掉cache，重新读取数采数据 */
        oal_dma_map_single(NULL, (oal_void *)(pst_daq_info->ul_daq_addr), pst_daq_info->ul_data_len, OAL_FROM_DEVICE);
#endif

        oal_netbuf_add_to_list_tail(pst_queue_netbuf, &g_st_data_acq.st_daq_workqueue.rx_daq_dbg_seq);

        oal_workqueue_schedule(&g_st_data_acq.st_daq_workqueue.rx_daq_work);
...
oal_uint32  dmac_config_data_acq(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param)
{
    ...
    switch(en_data_acq)
    {
        ...
        case DMAC_DATA_ACQ_COND:
             /* 配置PHY寄存器 */
             ul_ret = dmac_data_acq_condition(pst_mac_vap, puc_param);
...
oal_uint32  dmac_data_acq_exit(oal_void)
{
    ...
    oal_cancel_work_sync(&g_st_data_acq.st_daq_workqueue.rx_daq_work);
