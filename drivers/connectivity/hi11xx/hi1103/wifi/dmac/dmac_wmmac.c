                ...
                oal_netbuf_stru                *pst_net_buff)
{
    ...
    {
        OAM_ERROR_LOG0(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WMMAC, "{dmac_mgmt_tx_addts_req::wmmac switch is shut down}");
        return OAL_FAIL;
    }
                ...
                oal_netbuf_stru                *pst_net_buff)
{
    ...
    if(OAL_FALSE == g_en_wmmac_switch)
    {
        OAM_ERROR_LOG0(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WMMAC, "{dmac_mgmt_tx_delts::wmmac switch is shut down}");
        return OAL_FAIL;
    }
    ...
    if (IS_STA(&pst_dmac_vap->st_vap_base_info))
    {
        ...
        for (uc_ac_idx = 0; uc_ac_idx < WLAN_WME_AC_BUTT; uc_ac_idx++)
        {
            if(pst_ctx_action_event->uc_tsid == pst_dmac_user->st_user_base_info.st_ts_info[uc_ac_idx].uc_tsid)
            {
                ...
                if (MAC_TS_NONE == pst_dmac_user->st_user_base_info.st_ts_info[uc_ac_idx].en_ts_status)
                {
                    return OAL_FAIL;
                }
