...
oal_void dmac_test_lpm_smps_set_rate(mac_tx_ctl_stru *pst_tx_ctl, hal_tx_txop_alg_stru *pst_txop_alg,dmac_test_stub_opera_enum_uint8 en_opera)
{
    ...
    if(OAL_TRUE == pst_tx_ctl->bit_ismcast)
    {
        return;
    }
...
oal_uint32 dmac_test_lpm_send_nulldata(dmac_vap_stru *pst_dmac_vap,oal_uint8 uc_psm,oal_uint8* puc_da)
{
    ...
    /* 设置ADDR2为SA */
    oal_set_mac_addr((oal_netbuf_header(pst_null_data) + 10),mac_mib_get_StationID(&pst_dmac_vap->st_vap_base_info));
    ...
    pst_tx_ctrl->bit_is_first_msdu         = OAL_TRUE;
    ...
    if (WLAN_VAP_MODE_BSS_STA == pst_dmac_vap->st_vap_base_info.en_vap_mode)
    {
        pst_tx_ctrl->us_tx_user_idx       = pst_dmac_vap->st_vap_base_info.us_assoc_vap_id;
    ...
    pst_tx_ctrl->bit_ismcast                = OAL_FALSE;
    pst_tx_ctrl->bit_is_amsdu               = OAL_FALSE;
    pst_tx_ctrl->bit_is_get_from_ps_queue   = OAL_TRUE;
    ...
    pst_tx_ctrl->us_mpdu_payload_len               = 0;
...
{
    /* 获取设备 */
    if (OAL_SWITCH_ON == pst_hal_device->bit_al_tx_flag)
    {
