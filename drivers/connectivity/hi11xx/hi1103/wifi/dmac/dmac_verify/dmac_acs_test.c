        ...
        const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    pst_dmac_dev = dmac_res_get_mac_dev(0);
    pst_hal_device = DMAC_DEV_GET_MST_HAL_DEV(pst_dmac_dev);
    ...
    if (0 == aul_nums[2])/* CHIP TEST */
    {
        ...
        hal_set_ch_statics_period(pst_hal_device, 8000);
        hal_set_ch_measurement_period(pst_hal_device, 8);
    ...
    else if (1 == aul_nums[2]) /* tx time test */
    {
        ...
        hal_set_ch_statics_period(pst_hal_device, 1000 * aul_nums[0]);
        hal_set_ch_measurement_period(pst_hal_device, 8);
        hal_enable_ch_statics(pst_hal_device, 1);
        g_st_acs_verify_info.ul_test_mode = aul_nums[1];
        OAL_MEMZERO(&g_st_acs_verify_info.st_acs_stats_result, OAL_SIZEOF(g_st_acs_verify_info.st_acs_stats_result));
    ...
    else if (2 == aul_nums[2]) /* read register */
    {
        ...
        hal_reg_info(pst_hal_device, aul_nums[0], &g_st_acs_verify_info.ul_reg_data);
    }
...
oal_void    dmac_acs_channel_meas_comp_handler(hal_to_dmac_device_stru *pst_hal_device, hal_ch_statics_irq_event_stru *pst_mac_stats_result)
{
