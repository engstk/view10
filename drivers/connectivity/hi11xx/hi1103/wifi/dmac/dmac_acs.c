...
#define DMAC_ACS_MAX_RSP_SIZE 256
/*****************************************************************************
  2 全局变量定义
*****************************************************************************/
/***********************************************************
...
OAL_STATIC oal_void dmac_acs_setup_vap_info(dmac_acs_vap_info_stru *pst_info, mac_vap_stru *pst_vap)
{
    ...
    pst_info->en_band           = pst_vap->st_channel.en_band;
}
...
OAL_STATIC oal_void dmac_acs_setup_user_info(dmac_acs_user_info_stru *pst_info, mac_vap_stru *pst_vap, mac_user_stru *pst_user)
{
    ...
    pst_info->c_rx_rssi            = oal_get_real_rssi(MAC_GET_DMAC_USER(pst_user)->s_rx_rssi);
}
...
OAL_STATIC oal_void dmac_acs_do_vap_perf(mac_device_stru *pst_mac_dev, oal_void *p_acs, mac_acs_cmd_stru *pst_cmd)
{
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_mac_vap->st_mac_user_list_head))
    {
        ...
        {
            pst_user_internal->c_rx_rssi = oal_get_real_rssi(pst_dmac_user->s_rx_rssi);
        }
        ...
        for (uc_index = 0; uc_index < WLAN_TID_MAX_NUM; uc_index++)
        {
            ...
            /* 计算延迟, 精度1us */
            ul_time_delta_us = (oal_uint32)OAL_TIME_GET_RUNTIME(MAC_GET_CB_TIMESTAMP(pst_tx_ctrl), (oal_uint32)DMAC_TIME_USEC_INT64(&st_time));
...
OAL_STATIC oal_void  dmac_acs_do_avl_channel(mac_device_stru *pst_mac_dev, oal_void *p_acs, mac_acs_cmd_stru *pst_cmd)
{
    ...
    for (uc_idx = 0; uc_idx < MAC_CHANNEL_FREQ_2_BUTT; uc_idx++)
    {
        if(OAL_SUCC == mac_is_channel_idx_valid(WLAN_BAND_2G, uc_idx))
        {
            ...
            pst_rc_info = mac_get_channel_idx_rc_info(WLAN_BAND_2G, uc_idx);
            pst_chan->us_rc_flag = pst_rc_info ? pst_rc_info->uc_behaviour_bmap : 0;
            pst_chan++;
            uc_tot++;
        }
    }
...
#endif
            pst_rc_info = mac_get_channel_idx_rc_info(WLAN_BAND_5G, uc_idx);
            pst_chan->us_rc_flag = pst_rc_info ? pst_rc_info->uc_behaviour_bmap : 0;
            pst_chan->uc_channel_number = uc_chan_num;
            pst_chan++;
            uc_tot++;
...
OAL_STATIC oal_void  dmac_acs_do_get_net_info(mac_device_stru *pst_mac_dev, oal_void *p_acs, mac_acs_cmd_stru *pst_cmd)
{
    ...
    dmac_acs_vap_internal_stru       *pst_vap_internal;
    oal_dlist_head_stru              *pst_vap_list;
    oal_uint8                         auc_buf[DMAC_ACS_MAX_RSP_SIZE];
    ...
    oal_int32                         l_size = OAL_SIZEOF(dmac_acs_net_info_stru);
#if 0
    dmac_acs_user_info_stru          *pst_user_info;
    dmac_acs_user_internal_stru      *pst_user_internal;
    mac_user_stru                    *pst_user;
    oal_dlist_head_stru              *pst_user_list;
#endif
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_vap_list, &pst_acs->st_vap_dlist_head)
    {
        ...
        if ((oal_int32)OAL_SIZEOF(auc_buf) - l_size < (oal_int32)OAL_SIZEOF(dmac_acs_vap_info_stru))
        {
            break;
        }

        l_size += (oal_int32)OAL_SIZEOF(dmac_acs_vap_info_stru);
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_vap_list, &pst_acs->st_vap_dlist_head)
    {
        ...
        OAL_DLIST_SEARCH_FOR_EACH(pst_user_list, &pst_vap_internal->st_user_dlist_head)
        {
            ...
            {
            	OAM_WARNING_LOG1(0, OAM_SF_ACS, "{dmac_acs_do_get_net_info::get user[%d] null.}", pst_user_internal->us_assoc_id);
            	continue;
            }

            if ((oal_int32)OAL_SIZEOF(auc_buf) - l_size < (oal_int32)OAL_SIZEOF(dmac_acs_user_info_stru))
            {
                break;
            }

            l_size += (oal_int32)OAL_SIZEOF(dmac_acs_user_info_stru);
...
OAL_STATIC dmac_acs_vap_internal_stru*  dmac_acs_add_vap(mac_vap_stru *pst_vap)
{
    ...
    {
        pst_vap_internal = (dmac_acs_vap_internal_stru *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL, OAL_SIZEOF(dmac_acs_vap_internal_stru), OAL_TRUE);
        if (OAL_PTR_NULL != pst_vap_internal)
        {
...
OAL_STATIC dmac_acs_user_internal_stru*  dmac_acs_add_user(mac_vap_stru *pst_vap, mac_user_stru *pst_user)
{
    ...
    if (OAL_PTR_NULL != pst_vap_internal)
    {
        ...
        if (OAL_PTR_NULL == pst_user_internal)
        {
            pst_user_internal = (dmac_acs_user_internal_stru *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL, OAL_SIZEOF(dmac_acs_user_internal_stru), OAL_TRUE);
            if (OAL_PTR_NULL != pst_user_internal)
            {
...
oal_uint32 dmac_acs_2040_user_set_bw(dmac_vap_stru *pst_dmac_vap, mac_channel_stru *pst_channel)
{
    ...
    {
        OAM_ERROR_LOG0(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ACS, "{dmac_acs_2040_user_set_bw::pst_hal_device null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    return dmac_mgmt_switch_channel(pst_hal_device, pst_channel, OAL_FALSE);
}


oal_uint32 dmac_acs_2040_user_recover_40(mac_vap_stru *pst_vap)
{

    dmac_vap_stru              *pst_dmac_vap   = OAL_PTR_NULL;
    mac_channel_stru           *pst_channel;
    dmac_device_stru           *pst_dmac_device;
    dmac_acs_2040_user         *pst_acs_2040_user;

    pst_dmac_vap    = mac_res_get_dmac_vap(pst_vap->uc_vap_id);
    if (OAL_PTR_NULL == pst_dmac_vap)
    {
        OAM_ERROR_LOG0(pst_vap->uc_vap_id, OAM_SF_ACS, "{dmac_acs_2040_user_recover_40::pst_dmac_vap null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    pst_channel = &(pst_vap->st_channel);

    pst_dmac_device = dmac_res_get_mac_dev(pst_vap->uc_device_id);
    if (OAL_PTR_NULL == pst_dmac_device)
    {
       OAM_WARNING_LOG0(pst_vap->uc_vap_id, OAM_SF_ACS, "{dmac_acs_2040_user_recover_40::pst_dmac_device null.}");
       return OAL_ERR_CODE_PTR_NULL;
    }
    pst_acs_2040_user = &(pst_dmac_device->st_acs_2040_user);

    if(WLAN_BAND_WIDTH_20M == pst_acs_2040_user->uc_cur_bw && (WLAN_BAND_WIDTH_40PLUS == pst_channel->en_bandwidth || WLAN_BAND_WIDTH_40MINUS == pst_channel->en_bandwidth))
    {
        OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_ACS, "device%d do bw 40M mode \r\n", pst_vap->uc_device_id);
        dmac_acs_2040_user_set_bw(pst_dmac_vap, pst_channel);
        pst_acs_2040_user->uc_cur_bw = WLAN_BAND_WIDTH_40M;
    }
...
oal_uint32 dmac_acs_2040_user_change_bw(mac_vap_stru *pst_vap)
{
    dmac_vap_stru              *pst_dmac_vap   = OAL_PTR_NULL;
    mac_channel_stru           *pst_channel;
    dmac_device_stru           *pst_dmac_device;
    ...
    if (OAL_PTR_NULL == pst_dmac_vap)
    {
        OAM_ERROR_LOG0(pst_vap->uc_vap_id, OAM_SF_ACS, "{dmac_acs_2040_user_change_bw::pst_dmac_vap null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    pst_channel = &(pst_vap->st_channel);
    ...
    if (OAL_PTR_NULL == pst_dmac_device)
    {
       OAM_WARNING_LOG0(pst_vap->uc_vap_id, OAM_SF_ACS, "{dmac_acs_2040_user_change_bw::pst_dmac_device null.}");
       return OAL_ERR_CODE_PTR_NULL;
    }
    pst_acs_2040_user = &(pst_dmac_device->st_acs_2040_user);

    if(!pst_acs_2040_user->uc_switch)
    {
        return OAL_SUCC;
    }

    if(0 == pst_acs_2040_user->uc_40_user_count && (WLAN_BAND_WIDTH_40PLUS == pst_channel->en_bandwidth || WLAN_BAND_WIDTH_40MINUS == pst_channel->en_bandwidth)
        && WLAN_BAND_WIDTH_20M != pst_acs_2040_user->uc_cur_bw)
    {
        OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_ACS, "device%d do bw 20M mode \r\n", pst_vap->uc_device_id);
        en_bandwidth = pst_channel->en_bandwidth;
        pst_channel->en_bandwidth = WLAN_BAND_WIDTH_20M;
        dmac_acs_2040_user_set_bw(pst_dmac_vap, pst_channel);
        pst_channel->en_bandwidth = en_bandwidth;
        pst_acs_2040_user->uc_cur_bw = WLAN_BAND_WIDTH_20M;
    }
    else if(0 != pst_acs_2040_user->uc_40_user_count && WLAN_BAND_WIDTH_20M == pst_acs_2040_user->uc_cur_bw)
    {
        OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_ACS, "device%d do bw 40M mode \r\n", pst_vap->uc_device_id);
        dmac_acs_2040_user_set_bw(pst_dmac_vap, pst_channel);
        pst_acs_2040_user->uc_cur_bw = WLAN_BAND_WIDTH_40M;
...
oal_uint32 dmac_acs_2040_user_count(mac_vap_stru *pst_vap, mac_user_stru *pst_user, oal_bool_enum_uint8 en_add)
{
    ...
    if (OAL_PTR_NULL == pst_dmac_device)
    {
       OAM_WARNING_LOG0(pst_vap->uc_vap_id, OAM_SF_ACS, "{dmac_acs_2040_user_count::pst_dmac_device null.}");
       return OAL_ERR_CODE_PTR_NULL;
    }
    pst_acs_2040_user = &(pst_dmac_device->st_acs_2040_user);
    ...
    if(OAL_TRUE == en_add)
    {
        if(WLAN_BW_CAP_40M == pst_user->en_bandwidth_cap)
        {
            pst_acs_2040_user->uc_40_user_count++;
            if(pst_acs_2040_user->uc_40_user_count > WLAN_ASSOC_USER_MAX_NUM)
            {
                pst_acs_2040_user->uc_40_user_count = WLAN_ASSOC_USER_MAX_NUM;
            }
        }
        OAM_WARNING_LOG3(pst_vap->uc_vap_id, OAM_SF_ACS, "device: %d, 2G 40M user count: %d, chip bw: %d", pst_vap->uc_device_id, pst_acs_2040_user->uc_40_user_count, pst_channel->en_bandwidth);
        OAM_WARNING_LOG4(pst_vap->uc_vap_id, OAM_SF_ACS, "add user bw: %d mac: %xxxxxxx%x%x\r\n", pst_user->en_bandwidth_cap, pst_user->auc_user_mac_addr[0], pst_user->auc_user_mac_addr[4],pst_user->auc_user_mac_addr[5]);
    }
    else
    {
        if(WLAN_BW_CAP_40M == pst_user->en_bandwidth_cap)
        {
            if(pst_acs_2040_user->uc_40_user_count > 0)
            {
                pst_acs_2040_user->uc_40_user_count--;
            }
        }
        OAM_WARNING_LOG3(pst_vap->uc_vap_id, OAM_SF_ACS, "device: %d, 2G 40M user count: %d, chip bw: %d", pst_vap->uc_device_id, pst_acs_2040_user->uc_40_user_count, pst_channel->en_bandwidth);
        OAM_WARNING_LOG4(pst_vap->uc_vap_id, OAM_SF_ACS, "del user bw: %d mac: %xxxxxxx%x%x\r\n", pst_user->en_bandwidth_cap, pst_user->auc_user_mac_addr[0], pst_user->auc_user_mac_addr[4],pst_user->auc_user_mac_addr[5]);
    }

    return dmac_acs_2040_user_change_bw(pst_vap);
}


oal_uint32 dmac_acs_2040_user_add_handler(mac_vap_stru *pst_vap, mac_user_stru *pst_user)
{
    ...
    OAM_INFO_LOG4(pst_vap->uc_vap_id, OAM_SF_ACS, "dmac_acs_2040_user_add__handler:: assoc_id=%d, en_is_multi_user=%d, mac = XX:XX:XX:XX:%02X:%02X.",
    ...
    return dmac_acs_2040_user_count(pst_vap, pst_user, OAL_TRUE);
}


oal_uint32 dmac_acs_2040_user_del_handler(mac_vap_stru *pst_vap, mac_user_stru *pst_user)
{
    ...
    return dmac_acs_2040_user_count(pst_vap, pst_user, OAL_FALSE);
}


oal_uint32 dmac_acs_2040_user_init(oal_uint8 uc_device_id)
{
    dmac_device_stru                    *pst_dmac_device;
    dmac_acs_2040_user                  *pst_acs_2040_user;
    ...
    if (OAL_PTR_NULL == pst_dmac_device)
    {
       OAM_WARNING_LOG0(0, OAM_SF_ACS, "{dmac_acs_2040_user_init::pst_dmac_device null.}");
       return OAL_ERR_CODE_PTR_NULL;
    }
    pst_acs_2040_user = &(pst_dmac_device->st_acs_2040_user);

    oal_memset(pst_acs_2040_user, 0, OAL_SIZEOF(dmac_acs_2040_user));
    pst_acs_2040_user->uc_cur_bw = WLAN_BAND_WIDTH_40M;//初始40M,便于40M切换到20M;

    dmac_alg_register_add_user_notify_func(DMAC_ALG_ADD_USER_NOTIFY_HWBW_2040, dmac_acs_2040_user_add_handler);
    dmac_alg_register_del_user_notify_func(DMAC_ALG_DEL_USER_NOTIFY_HWBW_2040, dmac_acs_2040_user_del_handler);

    return OAL_SUCC;
}


oal_uint32 dmac_acs_2040_user_set_switch(oal_bool_enum_uint8 en_value, mac_vap_stru *pst_mac_vap)
{
    dmac_device_stru                    *pst_dmac_device;
    dmac_acs_2040_user                  *pst_acs_2040_user;

    OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ACS, "{dmac_acs_2040_user_set_switch %d}", en_value);

    pst_dmac_device = dmac_res_get_mac_dev(pst_mac_vap->uc_device_id);
    if (OAL_PTR_NULL == pst_dmac_device)
    {
       OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ACS, "{dmac_acs_2040_user_set_switch::pst_dmac_device null.}");
       return OAL_ERR_CODE_PTR_NULL;
    }
    pst_acs_2040_user = &(pst_dmac_device->st_acs_2040_user);

    pst_acs_2040_user->uc_switch = en_value;

    if(OAL_TRUE == en_value)
    {
        dmac_acs_2040_user_change_bw(pst_mac_vap);
    }
    else
    {
        dmac_acs_2040_user_recover_40(pst_mac_vap);
    }
...
#ifdef _PRE_WLAN_FEATURE_HWBW_20_40
    dmac_acs_2040_user_count(pst_vap, pst_user, OAL_TRUE);
#endif
...
#ifdef _PRE_WLAN_FEATURE_HWBW_20_40
    dmac_acs_2040_user_count(pst_vap, pst_user, OAL_FALSE);
#endif
...
oal_bool_enum_uint8  dmac_acs_get_rate_num(mac_user_stru            *pst_mac_user,
                                            ...
                                            oal_uint32                        *pul_num)
{
    ...
    {
        OAM_ERROR_LOG0(pst_mac_user->uc_vap_id, OAM_SF_ACS, "{dmac_acs_get_rate_num::vap id[%d]dmac_user_get_hal_device is NULL!}");
        return OAL_FALSE;
    }
                                                        ...
                                                        oal_uint32                        *pul_rate_kbps)
{
    ...
    if (OAL_TRUE != dmac_acs_get_rate_num(pst_mac_user, un_rate_info, &ul_rate_num))
    {
        return OAL_FALSE;
    }
...
oal_void  dmac_acs_tx_notify(dmac_acs_stru *pst_acs, oal_uint8 uc_vap_id, oal_netbuf_stru *pst_buf)
{
    ...
    if ((OAL_PTR_NULL != pst_vap_internal) && (OAL_TRUE == pst_vap_internal->en_dfs_wait_for_csa_sent))
    {
            ...
            && (OAL_TRUE == mac_ieeee80211_is_action(oal_netbuf_header(pst_buf))))
        {
            ...
            puc_frame_body  = mac_netbuf_get_payload(pst_buf);
...
oal_uint32  dmac_acs_tx_update(mac_user_stru *pst_mac_user, oal_netbuf_stru *pst_buf, hal_tx_dscr_ctrl_one_param *pst_tx_dscr_one)
{
    ...
    for (uc_rate_rank = 0; uc_rate_rank <= uc_last_rank; uc_rate_rank++)
    {
        ...
        en_found = dmac_acs_get_rate_kbps(pst_mac_device, pst_mac_user, pst_tx_dscr_one->ast_per_rate[uc_rate_rank], pst_tx_dscr_one->uc_bandwidth, &ul_rate_kbps);
...
#ifdef _PRE_WLAN_FEATURE_HWBW_20_40
    dmac_acs_2040_user_init(pst_device->uc_device_id);
#endif
