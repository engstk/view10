...
{
    mac_frame_he_mac_cap_stru          *pst_frame_he_mac_cap_info;
    mac_frame_he_phy_cap_stru          *pst_frame_he_phy_cap_info;
    mac_frame_he_tx_rx_mcs_nss_stru    *pst_frame_he_tx_rx_mcs_nss;
    oal_uint8                          *puc_he_buffer;
    oal_uint8                           uc_loop;
    oal_uint8                           uc_mcs_num;
    ...
    ***************************************************************************/
    ...
    pst_frame_he_mac_cap_info = (mac_frame_he_mac_cap_stru *)puc_he_buffer;
    puc_he_buffer += MAC_HE_MAC_CAP_LEN;
    oal_memcopy((oal_uint8 *)(&pst_he_cap_value->st_he_mac_cap),(oal_uint8 *)pst_frame_he_mac_cap_info,sizeof(mac_frame_he_mac_cap_stru));

    /*解析 HE PHY Capabilities Info*/
    pst_frame_he_phy_cap_info = (mac_frame_he_phy_cap_stru *)puc_he_buffer;
    puc_he_buffer += MAC_HE_PHY_CAP_LEN;
    oal_memcopy((oal_uint8 *)(&pst_he_cap_value->st_he_phy_cap),(oal_uint8 *)pst_frame_he_phy_cap_info,sizeof(mac_frame_he_phy_cap_stru));

    /*解析HE-MCS NSS */
    pst_frame_he_tx_rx_mcs_nss = (mac_frame_he_tx_rx_mcs_nss_stru *)puc_he_buffer;
    puc_he_buffer += MAC_HE_MCS_NSS_MIN_LEN;
    oal_memcopy((oal_uint8 *)(&pst_he_cap_value->st_he_tx_rx_mcs_nss),(oal_uint8 *)pst_frame_he_tx_rx_mcs_nss,sizeof(mac_frame_he_tx_rx_mcs_nss_stru));
    uc_mcs_num = 0;
    for(uc_loop =0;uc_loop < 5;uc_loop++)
    {
        if(1 == ((pst_frame_he_tx_rx_mcs_nss->bit_tx_bw_bitmap << uc_loop) & 0x1))
        {
            uc_mcs_num ++;
        }
    }
    oal_memcopy(pst_he_cap_value->st_tx_he_mcs,puc_he_buffer,OAL_SIZEOF(mac_frame_he_mcs_nss_stru)*uc_mcs_num);
    puc_he_buffer += (OAL_SIZEOF(mac_frame_he_mcs_nss_stru)*uc_mcs_num);

    uc_mcs_num = 0;
    for(uc_loop =0;uc_loop < 5;uc_loop++)
    {
        if(1 == ((pst_frame_he_tx_rx_mcs_nss->bit_rx_bw_bitmap << uc_loop) & 0x1))
        {
            uc_mcs_num ++;
        }
    }
    oal_memcopy(pst_he_cap_value->st_rx_he_mcs,puc_he_buffer,OAL_SIZEOF(mac_frame_he_mcs_nss_stru)*uc_mcs_num);
    puc_he_buffer += (OAL_SIZEOF(mac_frame_he_mcs_nss_stru)*uc_mcs_num);
...
oal_uint32  mac_ie_parse_he_oper(oal_uint8 *puc_he_oper_ie,mac_frame_he_oper_ie_stru *pst_he_oper_ie_value)
{
    ...
    mac_frame_he_operation_param_stru  *pst_he_oper_param;
    mac_frame_he_basic_mcs_nss_stru    *pst_he_basic_mcs_nss;
    ...
    ***************************************************************************/
    ...
    oal_memcopy((oal_uint8 *)(&pst_he_oper_ie_value->st_he_basic_mcs_nss),pst_he_basic_mcs_nss,OAL_SIZEOF(mac_frame_he_basic_mcs_nss_stru));

    if(1 == pst_he_oper_param->bit_vht_operation_info_present)
    {
