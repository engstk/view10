    ...
    if (pst_rrm_info->uc_local_pwr_constraint != pst_pwr_constraint->uc_local_pwr_constraint)
    {
        ...
        else
        {
            ...
            if (pst_rrm_info->uc_ori_max_reg_pwr > pst_rrm_info->uc_local_pwr_constraint)
            {
                ...
                /* 刷新发送功率 */
                dmac_pow_set_vap_tx_power(&pst_dmac_vap->st_vap_base_info, HAL_POW_SET_TYPE_REFRESH);
            }
        }
    }
    return OAL_SUCC;
...
oal_uint32 dmac_rrm_fill_basic_rm_rpt_action(dmac_vap_stru *pst_dmac_vap)
{
    ...
    {
        OAM_ERROR_LOG1(0, OAM_SF_TX, "{dmac_rrm_fill_basic_rm_rpt_action::pst_mac_user[%d] null.", pst_dmac_vap->st_vap_base_info.us_assoc_vap_id);
...
oal_void dmac_rrm_encap_meas_rpt(dmac_vap_stru *pst_dmac_vap)
{
    ...
    do {
        ...
        uc_curr_beacon_item_len = MAC_BEACON_RPT_FIX_LEN + (oal_uint8)pst_meas_rpt_bcn->ul_rpt_detail_act_len;
        ...
        oal_memcopy(pst_bcn_rpt_item->aul_act_meas_start_time, pst_meas_rpt_bcn_item->aul_act_meas_start_time,
        ...
        oal_memcopy(pst_bcn_rpt_item->auc_bssid, pst_meas_rpt_bcn_item->auc_bssid, OAL_SIZEOF(pst_meas_rpt_bcn_item->auc_bssid));
        ...
        if (pst_meas_rpt_bcn->ul_rpt_detail_act_len)
        {
            ...
            *(pst_bcn_rpt_item->auc_subelm + 1) = (oal_uint8)(pst_meas_rpt_bcn->ul_rpt_detail_act_len);
                                                       ...
                                                       oal_uint8 *puc_addr)
{
    ...
    oal_memcopy(pst_meas_rpt_bcn_item->aul_act_meas_start_time, pst_rrm_info->aul_act_meas_start_time,
    ...
    {
        ul_act_meas_lo = pst_meas_rpt_bcn_item->aul_act_meas_start_time[0];
        /*lint -save -e776 */
        pst_meas_rpt_bcn_item->us_duration = (oal_uint16)((((oal_uint64)(pst_meas_rpt_bcn_item->ul_parent_tsf + 1) << 32) - ul_act_meas_lo) >> 10);
        /*lint -restore */
    }
    ...
    /*lint -save -e571 */
    pst_meas_rpt_bcn_item->uc_rcpi                 = (oal_uint8)((oal_uint32)(pst_rx_ctrl->st_rx_statistic.c_rssi_dbm + 110) << 1);
    /*lint -restore */
                                                ...
                                                mac_meas_rpt_bcn_stru  *pst_meas_rpt_bcn)
{
    ...
    {
        OAM_WARNING_LOG0(0, OAM_SF_RRM, "{dmac_rrm_extract_bcn_rpt_detail::rpt detail is 0}");
        return;
    }

    /* 定长field拷贝 */
    uc_fix_len = MAC_TIME_STAMP_LEN + MAC_BEACON_INTERVAL_LEN + MAC_CAP_INFO_LEN;
    if (us_frame_len <= (MAC_80211_FRAME_LEN + uc_fix_len))
    {
        OAM_ERROR_LOG1(0, OAM_SF_RRM, "{dmac_rrm_encap_bcn_rpt_detail::us_frame_len[%d] err }", us_frame_len);
        return;
    }
    ...
    if (OAL_PTR_NULL == puc_rpt_detail_data)
    {
        ...
        OAL_MEM_INFO_PRINT(OAL_MEM_POOL_ID_LOCAL);
        OAL_MEM_USAGE_INFO_PRINT(OAL_MEM_POOL_ID_LOCAL);
        return;
    }
...
oal_void dmac_rrm_get_bcn_info_from_rx(dmac_vap_stru *pst_dmac_vap, oal_netbuf_stru  *pst_netbuf)
{
    ...
    if (!ETHER_IS_BROADCAST(pst_rrm_info->st_bcn_req_info.auc_bssid))
    {
        ...
        if (oal_memcmp(pst_rrm_info->st_bcn_req_info.auc_bssid, pst_frame_hdr->auc_address3, WLAN_MAC_ADDR_LEN))
        {
            return;
        }
    }
    ...
    if (!oal_dlist_is_empty(&(pst_rrm_info->st_meas_rpt_list)))
    {
        OAL_DLIST_SEARCH_FOR_EACH(pst_meas_rpt_entry, &(pst_rrm_info->st_meas_rpt_list))
        {
            ...
            if (0 == oal_memcmp(pst_meas_rpt_bcn_entry->pst_meas_rpt_bcn_item->auc_bssid, pst_frame_hdr->auc_address3, WLAN_MAC_ADDR_LEN))
            {
                return;
            }
        }
    ...
    if (0 != pst_rrm_info->st_bcn_req_info.uc_ssid_len)
    {
        ...
        || (0 != oal_memcmp(pst_rrm_info->st_bcn_req_info.puc_ssid, puc_ssid_ie, pst_rrm_info->st_bcn_req_info.uc_ssid_len)))
        {
            return;
        }
    }
...
oal_uint32 dmac_rrm_meas_bcn(dmac_vap_stru *pst_dmac_vap, mac_bcn_req_stru *pst_bcn_req, mac_scan_req_stru   *pst_scan_req)
{
    ...
    switch (pst_bcn_req->en_mode)
    {
        ...
        case RM_BCN_REQ_MEAS_MODE_PASSIVE:
            if (OAL_FALSE == mac_mib_get_dot11RMBeaconPassiveMeasurementActivated(&pst_dmac_vap->st_vap_base_info))
            {
        ...
        case RM_BCN_REQ_MEAS_MODE_ACTIVE:
            if (OAL_FALSE == mac_mib_get_dot11RMBeaconActiveMeasurementActivated(&pst_dmac_vap->st_vap_base_info))
            {
                if ( OAL_SUCC != dmac_rrm_fill_basic_rm_rpt_action(pst_dmac_vap))
                {
                    return OAL_FAIL;
        ...
        case RM_BCN_REQ_MEAS_MODE_TABLE:
            ...
            if (OAL_FALSE == mac_mib_get_dot11RMBeaconTableMeasurementActivated(&pst_dmac_vap->st_vap_base_info))
            {
                if ( OAL_SUCC != dmac_rrm_fill_basic_rm_rpt_action(pst_dmac_vap))
                {
                    return OAL_FAIL;
...
oal_uint32 dmac_rrm_parse_beacon_req(dmac_vap_stru *pst_dmac_vap, mac_meas_req_ie_stru  *pst_meas_req_ie)
{
    ...
    {
        if(OAL_SUCC != dmac_rrm_get_bcn_rpt_channels(pst_bcn_req, apst_ap_chn_rpt,  uc_ap_chn_rpt_count, &st_scan_req))
        {
            return OAL_FAIL;
        }
    }
    else
    {
        ...
        do {
            ...
            /*lint -save -e834 */
            c_ap_ch_rpt_search_len = (oal_int8)(c_bcn_req_sub_len - ((oal_uint8 *)apst_ap_chn_rpt[uc_ap_chn_rpt_count] - pst_bcn_req->auc_subelm) - (2 + apst_ap_chn_rpt[uc_ap_chn_rpt_count]->uc_length));
            /*lint -restore */
            uc_ap_chn_rpt_count++;
...
oal_uint32 dmac_rrm_get_mgmt_tx_pow(mac_user_stru *pst_user, wlan_channel_band_enum_uint8 en_freq_band,oal_uint8 *puc_tx_pow, oal_bool_enum_uint8 en_is_presicion_shift)
{
    ...
    {
        OAM_ERROR_LOG1(0, OAM_SF_TPC, "{dmac_rrm_get_mgmt_tx_pow::vap[%d]get pst_hal_device is NULL!}", pst_user->uc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    /* 获取管理帧的发送功率idx。2.4G:0 5G: 4 */
    if (WLAN_BAND_2G == en_freq_band)
    {
        uc_cur_rate_pow_idx = 0;
    }
    else
    {
        uc_cur_rate_pow_idx = 4;
    }

    /*输出*/
    hal_get_rate_idx_pow(pst_hal_device, uc_cur_rate_pow_idx, en_freq_band, &us_tx_pow);
    /*lint -save -e732 */
    us_tx_pow += HAL_DEV_GET_PER_RATE_TPC_GAIN_BY_LVL(pst_hal_device, en_freq_band, uc_cur_rate_pow_idx, 0);
    /*lint -restore */
    us_tx_pow  = (OAL_TRUE == en_is_presicion_shift) ? us_tx_pow:(us_tx_pow / HAL_POW_PRECISION_SHIFT);
...
oal_uint32 dmac_rrm_encap_link_meas_rpt(dmac_vap_stru *pst_dmac_vap, oal_netbuf_stru *pst_netbuf, oal_netbuf_stru *pst_rx_netbuf)
{
    ...
    ********************************************************************************/
    ...
    pst_regdom_info = mac_get_channel_num_rc_info(pst_dmac_vap->st_vap_base_info.st_channel.en_band, pst_dmac_vap->st_vap_base_info.st_channel.uc_chan_number);
    dmac_rrm_get_mgmt_tx_pow(pst_mac_user, pst_dmac_vap->st_vap_base_info.st_channel.en_band, &uc_tx_pow, OAL_FALSE);
    puc_payload[uc_offset++] = OAL_MIN(uc_tx_pow, (oal_uint8)OAL_MIN(pst_regdom_info->uc_max_reg_tx_pwr, pst_regdom_info->us_max_tx_pwr));
    ...
    /* RCPI */
    /*lint -save -e571 */
    puc_payload[uc_offset++]= (oal_uint8)((oal_uint32)(pst_cb_ctrl->st_rx_statistic.c_rssi_dbm + 110) << 1);
    /*lint -restore */
...
oal_void dmac_rrm_send_link_meas_rpt_action(dmac_vap_stru *pst_dmac_vap, oal_netbuf_stru *pst_rx_netbuf)
{
    ...
    if (OAL_TRUE != mac_mib_get_dot11RMLinkMeasurementActivated(&pst_dmac_vap->st_vap_base_info))
    {
