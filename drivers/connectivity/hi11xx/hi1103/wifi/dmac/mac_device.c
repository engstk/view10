...
#else
mac_blacklist_info_stru g_ast_blacklist[WLAN_SERVICE_DEVICE_SUPPORT_MAX_NUM_SPEC * WLAN_SERVICE_AP_MAX_NUM_PER_DEVICE];
#endif

oal_uint8 g_auc_valid_blacklist_idx[WLAN_SERVICE_DEVICE_SUPPORT_MAX_NUM_SPEC];
#endif
...
oal_void mac_blacklist_get_pointer(oal_uint8 uc_dev_id, oal_uint8 uc_chip_id, mac_blacklist_info_stru **pst_blacklist_info)
{
#if (_PRE_WLAN_FEATURE_BLACKLIST_LEVEL == _PRE_WLAN_FEATURE_BLACKLIST_VAP)
    oal_uint8      uc_device_index;
    oal_uint8      uc_vap_index;
    oal_uint16     us_array_index;
#endif
    ...
    *pst_blacklist_info = &g_ast_blacklist[uc_dev_id];
    ...
    us_array_index = uc_device_index * WLAN_SERVICE_AP_MAX_NUM_PER_DEVICE + uc_vap_index;

    if (us_array_index > WLAN_SERVICE_DEVICE_SUPPORT_MAX_NUM_SPEC * WLAN_SERVICE_AP_MAX_NUM_PER_DEVICE -1)
    {
        OAM_ERROR_LOG0(uc_vap_index, OAM_SF_ANY, "{hmac_blacklist_pointer_init::vap index wrong.}");
        return;
    }
