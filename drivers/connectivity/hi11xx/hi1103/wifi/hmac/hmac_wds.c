                ...
                oal_uint16          *pus_user_idx)
{
    ...
    ul_ret = mac_vap_find_user_by_macaddr(&(pst_hmac_vap->st_vap_base_info), pst_wds_sta->pst_related_node->auc_mac, pus_user_idx);
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG4(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY, "{hmac_find_valid_user_by_wds_sta::mac_vap_find_user_by_macaddr failed. mac[%02x:%02x:%02x:%02x]}", puc_sta_mac_addr[2], puc_sta_mac_addr[3], puc_sta_mac_addr[4], puc_sta_mac_addr[5]);
    }
    oal_rw_lock_write_unlock(&pst_hmac_vap->st_wds_table.st_lock);
                ...
                oal_uint8               *pst_addr)
{
    ...
    {
        OAM_ERROR_LOG4(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY, "{hmac_vap_set_wds_user::mac_vap_get_user_by_addr failed. mac[XX%02x:%02x:%02x%02x]}", pst_addr[2], pst_addr[3], pst_addr[4], pst_addr[5]);
        return OAL_FAIL;
    }
                ...
                oal_uint8               *pst_addr)
{
    ...
    if (OAL_UNLIKELY(OAL_SUCC != ul_ret))
    {
        OAM_ERROR_LOG4(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY, "{hmac_vmsta_get_user_a4_support::mac_vap_get_user_by_addr failed. mac[XX%02x:%02x:%02x%02x]}", pst_addr[2], pst_addr[3], pst_addr[4], pst_addr[5]);
        return OAL_FALSE;
    }
...
OAL_STATIC oal_uint32 hmac_wds_tx_event( hmac_vap_stru *pst_vap, hmac_user_stru *pst_user, oal_netbuf_stru *pst_buf)
{
    ...
    /* 封装802.11头 */
     ...
     if (OAL_UNLIKELY(OAL_SUCC != ul_ret))
     {
         OAM_WARNING_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX, "{hmac_wds_tx_event::frw_event_dispatch_event failed[%d].}", ul_ret);
         OAM_STAT_VAP_INCR(pst_vap->st_vap_base_info.uc_vap_id, tx_abnormal_msdu_dropped, 1);
     }
