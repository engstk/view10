...
oal_void  hmac_tx_ampdu_switch(oal_uint32 ul_tx_bytes, oal_uint32 ul_dur_time)
{
    ...
    if(0 != ul_dur_time)
    {
        ul_tx_throughput = (ul_tx_bytes >> 7) / ul_dur_time;
    }

    if (ul_tx_throughput > ul_limit_throughput_high)
    {
        /* 高于300M,切换硬件聚合 */
        en_ampdu_hw = OAL_TRUE;
    }
    else if (ul_tx_throughput < ul_limit_throughput_low)
    {
        /* 低于200M,切换硬件聚合 */
    ...
    OAM_WARNING_LOG4(0,OAM_SF_ANY,"{hmac_tx_ampdu_switch: limit_high = [%d],limit_low = [%d],tx_throught= [%d], continue_time = [%d]!}",
          ul_limit_throughput_high,ul_limit_throughput_low, ul_tx_throughput,ul_dur_time);
...
OAL_INLINE oal_uint32  hmac_tx_lan_to_wlan(mac_vap_stru *pst_vap, oal_netbuf_stru *pst_buf)
{
    ...
    if(OAL_TRUE==g_uc_print_host_wakeup_dev)
    {
        ...
        g_uc_print_host_wakeup_dev = OAL_FALSE;
    }
#endif
