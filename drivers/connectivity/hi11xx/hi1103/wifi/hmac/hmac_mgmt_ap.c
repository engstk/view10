        ...
        case MAC_ACTION_CATEGORY_PUBLIC:
        {
            ...
            switch (puc_data[MAC_ACTION_OFFSET_ACTION])
            {
                case MAC_PUB_VENDOR_SPECIFIC:
                {
                    ...
                    if (0 == oal_memcmp(puc_data  + MAC_ACTION_OFFSET_ACTION, g_auc_huawei_oui, MAC_OUI_LEN))
                    {
                       hmac_huawei_action_process(pst_hmac_vap, pst_netbuf, puc_data[MAC_ACTION_OFFSET_ACTION + MAC_OUI_LEN]);
                    }
