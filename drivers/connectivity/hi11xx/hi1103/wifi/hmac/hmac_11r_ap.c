...
oal_uint32 hmac_ft_assoc(mac_vap_stru *pst_mac_vap, oal_mlme_ie_stru *pst_mlme_ie)
{
    ...
    hmac_vap_stru                  *pst_hmac_vap;
    ul_rslt = mac_vap_find_user_by_macaddr(pst_mac_vap, pst_mlme_ie->auc_macaddr, &us_user_idx);
    if (OAL_SUCC != ul_rslt)
    {
    ...
    {
        ul_asoc_rsp_len = hmac_mgmt_encap_asoc_rsp_ap(pst_mac_vap,
    ...
    if (0 == ul_asoc_rsp_len)
    {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
                         "{hmac_ft_assoc::hmac_mgmt_encap_asoc_rsp_ap encap msg fail.}");
        oal_netbuf_free(pst_asoc_rsp);
    ...
    hmac_mgmt_reset_psm(pst_mac_vap, MAC_GET_CB_TX_USER_IDX(pst_tx_ctl));

    if (MAC_SUCCESSFUL_STATUSCODE == en_status_code)
    {
        hmac_user_set_asoc_state(pst_mac_vap, &pst_hmac_user->st_user_base_info, MAC_USER_STATE_ASSOC);
    }

    ul_rslt = hmac_tx_mgmt_send_event(pst_mac_vap, pst_asoc_rsp, (oal_uint16)ul_asoc_rsp_len);

    if (OAL_SUCC != ul_rslt)
    {
        OAM_WARNING_LOG1(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ASSOC,
                         "{hmac_ft_assoc::hmac_tx_mgmt_send_event failed[%d].}", ul_rslt);
        oal_netbuf_free(pst_asoc_rsp);
    ...
    if (MAC_SUCCESSFUL_STATUSCODE == en_status_code)
    {
        ...
        ul_rslt = hmac_config_user_rate_info_syn(pst_mac_vap, &pst_hmac_user->st_user_base_info);
        if (OAL_SUCC != ul_rslt)
        {
            OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
                           "{hmac_ft_assoc::hmac_config_user_rate_info_syn failed[%d].}", ul_rslt);
        }

        /*  user已经关联上，抛事件给DMAC，在DMAC层挂用户算法钩子 */
        hmac_user_add_notify_alg(pst_mac_vap, us_user_idx);
        for (uc_chip_id = 0; uc_chip_id < WLAN_CHIP_MAX_NUM_PER_BOARD; uc_chip_id++)
        {
            if (uc_chip_id != pst_mac_vap->uc_chip_id)
            {
                ...
                {
                    pst_hmac_other_user = mac_res_get_hmac_user(us_other_user_idx);
                    if (OAL_PTR_NULL != pst_hmac_other_user)
                    {
                        ...
                        /* 抛事件上报内核，已经删除某个STA */
                        hmac_mgmt_send_disassoc_frame(&(pst_hmac_other_vap->st_vap_base_info), pst_mlme_ie->auc_macaddr, MAC_DISAS_LV_SS, OAL_FALSE);
                        hmac_handle_disconnect_rsp_ap(pst_hmac_other_vap, pst_hmac_other_user);
                        hmac_user_del(&(pst_hmac_other_vap->st_vap_base_info), pst_hmac_other_user);
                    }
                }
            }
        }
    ...
    hmac_config_vap_info(pst_mac_vap, OAL_SIZEOF(oal_uint32), (oal_uint8 *)&ul_rslt);
...
oal_uint16  hmac_ft_encap_auth_rsp(mac_vap_stru *pst_mac_vap, oal_netbuf_stru *pst_auth_rsp, oal_mlme_ie_stru *pst_mlme)
{
    ...
    if(mac_addr_is_zero(pst_mlme->auc_macaddr))
    {
    ...
    /* 获取hmac user指针 */
    ul_ret = mac_vap_find_user_by_macaddr(pst_mac_vap, pst_mlme->auc_macaddr, &us_user_index);
    if (OAL_SUCC != ul_ret)
    {
...
oal_uint32 hmac_ft_auth(mac_vap_stru *pst_mac_vap, oal_mlme_ie_stru *pst_mlme_ie)
{
    ...
    pst_tx_ctl = (mac_tx_ctl_stru *)oal_netbuf_cb(pst_auth_rsp);
    if (OAL_PTR_NULL != mac_res_get_hmac_user(MAC_GET_CB_TX_USER_IDX(pst_tx_ctl)))
    {
        hmac_mgmt_reset_psm(pst_mac_vap, MAC_GET_CB_TX_USER_IDX(pst_tx_ctl));
    }

    /* 抛事件给dmac发送认证帧 */
    ul_ret = hmac_tx_mgmt_send_event(pst_mac_vap, pst_auth_rsp, us_auth_rsp_len);
    if (OAL_SUCC != ul_ret)
    {
        oal_netbuf_free(pst_auth_rsp);
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_AUTH, "{hmac_ft_auth::hmac_tx_mgmt_send_event failed[%d].}", ul_ret);
    }
    return OAL_SUCC;
}
