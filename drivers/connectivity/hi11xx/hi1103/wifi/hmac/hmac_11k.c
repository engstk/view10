...
oal_uint32 hmac_rrm_proc_neighbor_request(hmac_vap_stru* pst_hmac_vap, hmac_user_stru *pst_hmac_user, oal_netbuf_stru *pst_netbuf)
{
    ...
    if(us_framebody_len > MAC_NEIGHBOR_REPORT_ACTION_REQ_FIX_LEN)
    {
        ...
        puc_ssid_sub_element = mac_find_ie(MAC_EID_SSID, puc_ssid_search_addr, us_framebody_len - MAC_NEIGHBOR_REPORT_ACTION_REQ_FIX_LEN);
        ...
        mac_rrm_req_cfg_stru *pst_req_cfg)
{
    ...
    ul_ret = hmac_tx_mgmt_send_event(pst_mac_vap, pst_mgmt_buf, us_radio_meas_req_frm_len);
    if (OAL_SUCC != ul_ret)
    {
        oal_netbuf_free(pst_mgmt_buf);
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_RRM, "{hmac_config_send_meas_req::hmac_tx_mgmt_send_event failed[%d].}", ul_ret);
        return ul_ret;
    }
...
OAL_STATIC oal_uint32 hmac_rrm_fill_basic_rm_rpt_action(hmac_vap_stru *pst_hmac_vap)
{
    ...
    pst_mac_user = mac_res_get_mac_user(pst_rrm_info->us_req_user_id);
...
OAL_STATIC oal_uint32 hmac_rrm_send_rm_rpt_action(hmac_vap_stru* pst_hmac_vap)
{
    ...
    /* 调用发送管理帧接口 */
    ul_ret = hmac_tx_mgmt_send_event(&(pst_hmac_vap->st_vap_base_info), pst_mgmt_buf, pst_hmac_vap->pst_rrm_info->us_rm_rpt_action_len);
    if (OAL_SUCC != ul_ret)
    {
        oal_netbuf_free(pst_mgmt_buf);
        pst_hmac_vap->pst_rrm_info->pst_rm_rpt_mgmt_buf = OAL_PTR_NULL;
        ...
        mac_action_rm_req_stru *pst_rm_req)
{
    ...
    /* 调用发送管理帧接口 */
    ul_ret = hmac_tx_mgmt_send_event(&(pst_hmac_vap->st_vap_base_info), pst_mgmt_buf, us_rm_rpt_action_len);
    if (OAL_SUCC != ul_ret)
    {
        oal_netbuf_free(pst_mgmt_buf);
...
OAL_STATIC oal_uint32  hmac_rrm_chn_load_scan_do(hmac_vap_stru *pst_hmac_vap, mac_chn_load_req_stru *pst_chn_load_req)
{
    ...
    ul_ret = mac_get_channel_idx_from_num(en_chan_band, pst_chn_load_req->uc_channum, &st_scan_req.ast_channel_list[0].uc_chan_idx);
    if (OAL_SUCC != ul_ret)
    {
    ...
    if(OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(0, OAM_SF_RRM, "hmac_rrm_chn_load_scan_do:hmac_scan_proc_scan_req_event failed, ret=%d", ul_ret);
        /*扫描失败回复refuse bit*/
        OAL_MEMZERO(&st_rptmode, OAL_SIZEOF(mac_meas_rpt_mode_stru));
        st_rptmode.bit_refused = 1;
        hmac_rrm_encap_meas_rpt_reject(pst_hmac_vap, &st_rptmode);
...
OAL_STATIC oal_uint32 hmac_rrm_bcn_scan_do(hmac_vap_stru *pst_hmac_vap, mac_bcn_req_stru *pst_bcn_req, mac_scan_req_stru *pst_scan_req)
{
    ...
    /* 直接调用扫描模块扫描请求处理函数 */
    ul_ret = hmac_scan_proc_scan_req_event(pst_hmac_vap, pst_scan_req);
    if(OAL_SUCC != ul_ret)
    {
...
oal_uint32 hmac_rrm_meas_bcn(hmac_vap_stru *pst_hmac_vap, mac_bcn_req_stru *pst_bcn_req, mac_scan_req_stru   *pst_scan_req)
{
    ...
    switch (pst_bcn_req->en_mode)
    {
        ...
        case RM_BCN_REQ_MEAS_MODE_TABLE:
            ...
            hmac_scan_rrm_proc_save_bss(&(pst_hmac_vap->st_vap_base_info), OAL_SIZEOF(st_trans_req_info), (oal_uint8 *)&st_trans_req_info);
            break;
...
oal_uint32 hmac_rrm_get_bcn_rpt_channels(mac_bcn_req_stru *pst_bcn_req, mac_ap_chn_rpt_stru **ppst_ap_chn_rpt, oal_uint8 uc_ap_chn_rpt_num, mac_scan_req_stru *pst_scan_req, wlan_channel_band_enum_uint8 en_chan_band)
{
    ...
    if (0 == pst_bcn_req->uc_channum)
    {
        ...
        {
            /* 判断信道是不是在管制域内 */
            if (OAL_SUCC == mac_is_channel_idx_valid(en_chan_band, uc_chan_idx))
            {
    ...
    else if (255 == pst_bcn_req->uc_channum)
    {
        ...
        for (uc_ap_chan_rpt_count = 0; uc_ap_chan_rpt_count < uc_ap_chn_rpt_num; uc_ap_chan_rpt_count++)
        {
            ...
            for (uc_chan_idx = 0; uc_chan_idx < uc_ap_chan_num; uc_chan_idx++)
            {
                ...
                if(OAL_SUCC != mac_get_channel_idx_from_num(en_chan_band, uc_chan_num, &uc_ap_chan_idx))
                {
                    continue;
                }

                /* 检查信道号是否有效 */
                if (OAL_SUCC != mac_is_channel_idx_valid(en_chan_band, uc_ap_chan_idx))
                {
                    continue;
                }
                pst_scan_req->ast_channel_list[uc_chan_avail_idx].uc_chan_number = uc_chan_num;
    ...
    else
    {
        ...
        en_chan_band = mac_get_band_by_channel_num(uc_chan_num);
        ul_ret = mac_get_channel_idx_from_num(en_chan_band, uc_chan_num, &uc_ap_chan_idx);
        if(ul_ret != OAL_SUCC)
        {
            OAM_WARNING_LOG1(0, OAM_SF_RRM, "hmac_rrm_get_bcn_rpt_channels: mac_get_channel_idx_from_num fail,error_code=%d", ul_ret);
            return ul_ret;
        }
        /* 临时调试，需要更新管制类表格 */
...
OAL_STATIC oal_void hmac_rrm_parse_beacon_req(hmac_vap_stru *pst_hmac_vap, mac_meas_req_ie_stru  *pst_meas_req_ie)
{
    ...
    else
    {
        ...
        puc_rpt_info_sub_element = mac_find_ie(1, puc_rpt_info_search_addr, c_bcn_req_sub_len);
        ...
        puc_ssid_sub_element = mac_find_ie(MAC_EID_SSID, puc_ssid_search_addr, c_bcn_req_sub_len);
        if (OAL_PTR_NULL != puc_ssid_sub_element)
        {
...
OAL_STATIC oal_void hmac_rrm_get_neighbor_rpt_channels(mac_scan_req_stru *pst_scan_req)
{
    ...
    for (uc_chan_idx = 0; uc_chan_idx < MAC_CHANNEL_FREQ_5_BUTT; uc_chan_idx++)
    {
        ...
        mac_get_channel_num_from_idx(WLAN_BAND_5G, uc_chan_idx, &uc_chan_num);
        pst_scan_req->ast_channel_list[uc_chan_avail_idx].uc_chan_number = uc_chan_num;
                 ...
                 oal_bool_enum_uint8 en_local_meas)
{
    ...
    if(OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(0, OAM_SF_RRM, "hmac_rrm_neighbor_scan_do:hmac_scan_proc_scan_req_event failed, ret=%d", ul_ret);
        if ( OAL_FALSE == en_local_meas )
        {
