...
//    oal_uint8               uc_mgmt_frm_type;
    ...
    puc_opmode_notify_ie = mac_find_ie(MAC_EID_OPMODE_NOTIFY, puc_payload_offset, (oal_int32)ul_msg_len);
    ...
    if ((OAL_PTR_NULL != puc_opmode_notify_ie) && (puc_opmode_notify_ie[1] == MAC_OPMODE_NOTIFY_LEN))
    {
        ...
        {
            OAM_WARNING_LOG1(pst_mac_user->uc_vap_id, OAM_SF_OPMODE, "{hmac_check_opmode_notify::mac_ie_proc_opmode_field failed[%d].}", ul_relt);
            return ul_relt;
        }
    }
    return OAL_SUCC;
...
oal_uint32  hmac_mgmt_rx_opmode_notify_frame(hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user, oal_netbuf_stru *pst_netbuf)
{
    mac_opmode_notify_stru     *pst_opmode_notify = OAL_PTR_NULL;
//    oal_uint8                   uc_mgmt_frm_type;
    ...
    {
        OAM_ERROR_LOG3(0, OAM_SF_OPMODE, "{hmac_mgmt_rx_opmode_notify_frame::pst_hmac_vap = [%x],pst_hmac_user = [%x], pst_netbuf = [%x]!}\r\n",
    ...
    {
        OAM_INFO_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_OPMODE, "{hmac_mgmt_rx_opmode_notify_frame::the vap is not support OperatingModeNotification!}\r\n");
        return OAL_SUCC;
    }
    ...
    {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_OPMODE, "{hmac_mgmt_rx_opmode_notify_frame::pst_mac_user null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    {
        OAM_WARNING_LOG1(pst_mac_user->uc_vap_id, OAM_SF_OPMODE, "{hmac_mgmt_rx_opmode_notify_frame::hmac_config_send_event failed[%d].}", ul_ret);
        return ul_ret;
    }
