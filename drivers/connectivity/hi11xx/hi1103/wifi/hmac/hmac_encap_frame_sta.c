...
oal_uint32 hmac_mgmt_encap_asoc_req_sta(hmac_vap_stru *pst_hmac_sta, oal_uint8 *puc_req_frame, oal_uint8 *puc_curr_bssid)
{
    ...
    if((OAL_PTR_NULL != pst_scaned_bss) && (OAL_TRUE == pst_scaned_bss->st_bss_dscr_info.en_support_rrm)
#ifndef _PRE_WLAN_FEATURE_11KV_INTERFACE
    /* 需要由上层接口修改IE,故无论bit_11k_enable为何值,都需要调用接口修改该IE内容,移除if语句 */
        && (OAL_TRUE == pst_hmac_sta->bit_11k_enable)
#endif
    )
    {
