                                                                 ...
                                                                 oal_uint16           us_frame_len)
{
    ...
    if ((OAL_PTR_NULL != puc_ie) && (puc_ie[1] >= MAC_HE_CAP_MIN_LEN))
    {
        ...
        if(0 == st_he_cap_value.st_he_phy_cap.bit_dual_band_support)
        {
            pst_bss_dscr->en_bw_cap = WLAN_BW_CAP_40M;
        }
        else
        {
            if(st_he_cap_value.st_he_phy_cap.bit_channel_width_set & 0x4)
            {
                pst_bss_dscr->en_bw_cap = WLAN_BW_CAP_80PLUS80;
            }
            else if(st_he_cap_value.st_he_phy_cap.bit_channel_width_set & 0x2)
            {
                pst_bss_dscr->en_bw_cap = WLAN_BW_CAP_160M;
            }
            else if(st_he_cap_value.st_he_phy_cap.bit_channel_width_set & 0x1)
            {
                pst_bss_dscr->en_bw_cap = WLAN_BW_CAP_40M;
            }
            else
...
OAL_STATIC OAL_INLINE oal_void  hmac_scan_update_bss_list_protocol(mac_bss_dscr_stru *pst_bss_dscr,
                                                                            oal_uint8         *puc_frame_body,
                                                                            oal_uint16         us_frame_len)
{
    ...
    /* 11ac */
    hmac_scan_update_bss_list_11ac(pst_bss_dscr,  puc_frame_body, us_frame_len);
