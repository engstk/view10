...
oal_uint32  hmac_car_user_bw_limit(hmac_vap_stru *pst_hmac_vap, mac_cfg_car_stru *pst_car_cfg_param)
{
    ...
    pst_hmac_user = mac_vap_get_hmac_user_by_addr(&pst_hmac_vap->st_vap_base_info, pst_car_cfg_param->auc_user_macaddr);
    if (OAL_PTR_NULL == pst_hmac_user)
    {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_MULTI_TRAFFIC, "{hmac_car_user_bw_limit: mac_vap_find_user_by_macaddr failed}");
        return OAL_ERR_CODE_PTR_NULL;
    }
...
oal_uint32  hmac_car_enable_switch(hmac_device_stru *pst_hmac_dev, mac_cfg_car_stru *pst_car_cfg_param)
{
    ...
    if (OAL_TRUE == pst_hmac_dev->en_car_enable_flag)
    {
        ...
        {
            frw_timer_restart_timer(&pst_hmac_dev->st_car_timer, pst_hmac_dev->us_car_ctl_cycle_ms, OAL_TRUE);
            //car 丢包统计清空
                                    ...
                                    oal_netbuf_stru *pst_netbuf, hmac_car_up_down_type_enum_uint8 en_car_type)
{
    ...
    //获取发送端的hmac_user
    pst_hmac_user = (hmac_user_stru *)mac_res_get_hmac_user(us_ta_user_idx);
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_hmac_user))
    {
        OAM_WARNING_LOG1(0, OAM_SF_CAR, "{hmac_car_process_uplink::mac_res_get_hmac_user error, user id=%d}", us_ta_user_idx);
        return OAL_ERR_CODE_PTR_NULL;
    }
