...
oal_netbuf_stru* hmac_defrag_process(hmac_user_stru *pst_hmac_user, oal_netbuf_stru *pst_netbuf, oal_uint32 ul_hrdsize)
{
    ...
    {
        oal_netbuf_free(pst_netbuf);
        OAM_WARNING_LOG0(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ANY, "{hmac_defrag_process::mac head len is 0.}");
        return OAL_PTR_NULL;
    }
    ...
    else
    {
        ...
        if (OAL_SUCC != ul_ret)
        {
            ...
            OAM_WARNING_LOG1(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ANY, "{hmac_defrag_process::hmac_de_mic failed[%d].}", ul_ret);
            return OAL_PTR_NULL;
        }
