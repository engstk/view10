                ...
                oal_bool_enum_uint8        *pen_is_ba_buf)
{
    ...
    pst_vap = (mac_vap_stru*)mac_res_get_mac_vap(pst_cb_ctrl->uc_mac_vap_id);
    ...
    {
        OAM_WARNING_LOG1(pst_cb_ctrl->uc_mac_vap_id, OAM_SF_BA, "{hmac_ba_filter_serv::ba_status = %d.", pst_ba_rx_hdl->en_ba_status);
        return OAL_SUCC;
    }
    ...
    {
        OAM_WARNING_LOG1(pst_cb_ctrl->uc_mac_vap_id, OAM_SF_BA, "{hmac_ba_filter_serv::We get a frag_frame[seq_num=%d] When BA_session is set UP!", us_seq_num);
        return OAL_SUCC;
    }
...
oal_uint32  hmac_ba_timeout_fn(oal_void *p_arg)
{
    ...
    {
        OAM_WARNING_LOG1(0, OAM_SF_BA, "{hmac_ba_timeout_fn::pst_hmac_user null. user idx %d.}", pst_alarm_data->us_mac_user_idx);
        return OAL_ERR_CODE_PTR_NULL;
    }
                ...
                hmac_tid_stru     *pst_tid_info)
{
    ...
    if(OAL_TRUE == MAC_BTCOEX_CHECK_VALID_STA(&(pst_hmac_vap->st_vap_base_info)))
    {
        ...
        {
            OAM_WARNING_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_BA, "{hmac_mgmt_check_set_rx_ba_ok::btcoex blacklist user, not addba!");
            return MAC_REQ_DECLINED;
        }
    }
#endif
