                ...
                oal_uint8                        *puc_param)
{
    ...
    if(OAL_TRUE == g_uc_print_host_wakeup_dev)
    {
...
#endif
    st_start_vap_param.l_ifindex = pst_mac_vap->l_ifindex;
...
oal_uint32  hmac_config_start_vap(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    pst_hmac_vap->st_vap_base_info.l_ifindex = pst_start_vap_param->l_ifindex;
...
oal_uint32  hmac_config_priv_set_mode(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++)
    {
        ...
        if ((WLAN_VHT_MODE == pst_vap->en_protocol)
            && (WLAN_BAND_2G == pst_vap->st_channel.en_band))
        {
            mac_vap_set_11ac2g(pst_vap, OAL_TRUE);
        }
        else
        {
            mac_vap_set_11ac2g(pst_vap, OAL_FALSE);
        }
#endif
...
//#endif
    ...
    OAL_IO_PRINT("hmac_config_set_beacon::pst_mac_vap->st_cap_flag.bit_11ac2g=%d, bit_disable_2ght40=%d\n",
    pst_mac_vap->st_cap_flag.bit_11ac2g,
    pst_mac_vap->st_cap_flag.bit_disable_2ght40);
#endif
...
oal_uint32  hmac_config_amsdu_ampdu_switch(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    g_uc_host_large_amsdu_enable = (oal_uint8)pst_amsdu_ampdu_cfg->ul_value;
#endif
...
oal_uint32  hmac_config_sdio_pcie_switch(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    mac_chip_stru              *pst_mac_chip;
    mac_cfg_set_tlv_stru       *pst_sdio_pcie_cfg;

    pst_mac_chip = hmac_res_get_mac_chip(pst_mac_vap->uc_chip_id);
    if (OAL_PTR_NULL == pst_mac_chip)
    {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_sdio_pcie_switch::pst_mac_chip null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    pst_sdio_pcie_cfg = (mac_cfg_set_tlv_stru *)puc_param;

    pst_mac_chip->en_hcc_chn_switch = (oal_uint8)pst_sdio_pcie_cfg->ul_value;
    OAM_WARNING_LOG1(0, OAM_SF_CFG, "hmac_config_sdio_pcie_switch: hcc_chn_switch = %d. 0:close 1:open",  pst_mac_chip->en_hcc_chn_switch);
    return OAL_SUCC;
}
...
oal_uint32  hmac_config_connect(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
   ...
   if(OAL_TRUE == pst_hmac_vap->bit_11r_enable)
   {
    ...
    if (MAC_VAP_STATE_UP == pst_mac_vap->en_vap_state)
    {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_connect:: roaming AP with ressoc frame}");
        if (!oal_memcmp(pst_mac_vap->auc_bssid, pst_connect_param->puc_bssid, OAL_MAC_ADDR_LEN)) {
            /* reassociation */
...
oal_uint32  hmac_11v_cfg_bsst_switch(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    oal_bool_enum_uint8     en_11v_active_flag = OAL_FALSE;
    /* 调用接口配置11v特性开关 */
    mac_mib_set_MgmtOptionBSSTransitionActivated(pst_mac_vap,en_11v_cfg_switch);
    /* 获取一把校验下 */
    en_11v_active_flag = mac_mib_get_MgmtOptionBSSTransitionActivated(pst_mac_vap);

    if (OAL_UNLIKELY(en_11v_active_flag != en_11v_cfg_switch))
    {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_11v_cfg_bsst_switch::config 11v feature bss transition capability failed.}");
        return OAL_FAIL;
    }
...
#ifdef _PRE_WLAN_FEATURE_11AX
    mac_mib_set_HEOptionImplemented(pst_mac_vap,pst_mib_update_info->en_11ax_cap);
#endif
...
oal_uint32 hmac_config_d2h_vap_m2s_info_syn(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param)
{
    ...
    if ((MAC_VAP_INVAILD == pst_mac_vap->uc_init_flag)||(OAL_PTR_NULL == pst_mac_vap->pst_mib_info)||(OAL_PTR_NULL == puc_param))
    {
        OAM_WARNING_LOG3(0, OAM_SF_M2S, "{hmac_config_vap_m2s_info_syn::pst_mac_vap->uc_init_flag[%d], pst_mac_vap->pst_mib_info[%p], puc_param[%p]!}",
            pst_mac_vap->uc_init_flag, pst_mac_vap->pst_mib_info, puc_param);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if(WLAN_M2S_TYPE_HW == pst_syn_info->en_m2s_type)
    {
        ...
        if(OAL_SUCC != ul_ret)
        {
            OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_M2S, "{hmac_config_vap_m2s_info_syn::hmac_config_kick_user fail!}");
        }
    ...
    OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_M2S,
                       "{hmac_config_vap_m2s_info_syn::en_vap_rx_nss:[%d],en_sm_power_save:[%d],en_m2s_type:[%d].}",
...
oal_uint32  hmac_config_send_neighbor_req(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    /*if(pst_mac_vap->st_ftm.en_lci_ie == OAL_TRUE)
    {*/
        puc_data[us_index++] = MAC_EID_MEASREQ;                   /* Element ID */
        puc_data[us_index++] = MAC_2040_COEX_LEN;                 /* Length */
        puc_data[us_index++] = 1;                                 /* Measurement Token */
        puc_data[us_index++] = 0;                                 /* Measurement Req Mode */
        puc_data[us_index++] = RM_RADIO_MEASUREMENT_LCI;               /* Measurement Type */

//    }
//
//    /* 封装Measurement Request IE  */
//    if(pst_mac_vap->st_ftm.en_location_civic_ie == OAL_TRUE)
//    {
        puc_data[us_index++] = MAC_EID_MEASREQ;                   /* Element ID */
        puc_data[us_index++] = MAC_2040_COEX_LEN;                 /* Length */
        puc_data[us_index++] = 1;                                 /* Measurement Token */
        puc_data[us_index++] = 0;                                 /* Measurement Req Mode */
        puc_data[us_index++] = RM_RADIO_MEASUREMENT_LOCATION_CIVIC;    /* Measurement Type */
    /*}*/
...
oal_uint32  hmac_config_set_tlv_cmd(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    switch (pst_config_para->us_cfg_id)
    {
        ...
        case WLAN_CFGID_SET_RX_AMPDU_AMSDU:
            ...
            /* 1103 mpw2 由于存在译码错误导致去使能amsdu默认打开可能导致死机 默认是关闭的
            这里需要动态开关该能力位 故需要抛事件去dmac处理 */
            break;
        case WLAN_CFGID_SET_SDIO_PCIE_SWICTCH:
            hmac_config_sdio_pcie_switch(pst_mac_vap, us_len, puc_param);
            return OAL_SUCC;
