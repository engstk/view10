...
#undef  THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_DMAC_ARP_OFFLOAD_ROM_C
...
dmac_arp_offload_cb  g_st_dmac_arp_offload_rom_cb = {OAL_PTR_NULL,
                                                     OAL_PTR_NULL,
                                                     OAL_PTR_NULL,
                                                     OAL_PTR_NULL};
...
oal_uint32 dmac_ao_encap_ieee80211_header(dmac_vap_stru *pst_dmac_vap,
                                                     ...
                                                     oal_netbuf_stru *pst_tx_buf)
{
    ...
    *(oal_uint16 *)pst_tx_hdr = pst_mac_user->st_cap_info.bit_qos ? ((oal_uint16)(WLAN_PROTOCOL_VERSION | WLAN_FC0_TYPE_DATA | WLAN_FC0_SUBTYPE_QOS) | us_tx_direction)
    ...
    if (WLAN_VAP_MODE_BSS_STA == pst_mac_vap->en_vap_mode)
    {
        ...
        oal_set_mac_addr(pst_tx_hdr->auc_address2, mac_mib_get_StationID(pst_mac_vap));
    ...
    else if (WLAN_VAP_MODE_BSS_AP == pst_mac_vap->en_vap_mode)
    {
        ...
        oal_set_mac_addr(pst_tx_hdr->auc_address3, mac_mib_get_StationID(pst_mac_vap));
    }
    ...
    {
        ((mac_ieee80211_qos_frame_stru *)pst_tx_hdr)->bit_qc_tid                    = WLAN_TIDNO_BEST_EFFORT;
    ...
    if (OAL_PTR_NULL != g_st_dmac_arp_offload_rom_cb.ao_encap_ieee80211_head_cb)
    {
        g_st_dmac_arp_offload_rom_cb.ao_encap_ieee80211_head_cb(pst_dmac_vap,pst_dmac_user,pst_rx_hdr,pst_tx_buf);
    }
    return OAL_SUCC;
}


oal_uint32 dmac_ao_encap_cb(dmac_vap_stru *pst_dmac_vap,
                                       dmac_user_stru *pst_dmac_user,
                                       oal_netbuf_stru *pst_tx_buf,
                                       ...
                                       oal_uint8 *puc_dst_mac_addr)
{
    ...
    if (OAL_TRUE == pst_mac_user->st_cap_info.bit_qos)
    {
        MAC_SET_CB_IS_QOS_DATA(pst_tx_ctl, OAL_TRUE);
        MAC_GET_CB_FRAME_HEADER_LENGTH(pst_tx_ctl)    = MAC_80211_QOS_FRAME_LEN;
        MAC_GET_CB_WME_AC_TYPE(pst_tx_ctl)   = WLAN_WME_AC_BE;
        MAC_GET_CB_WME_TID_TYPE(pst_tx_ctl) = WLAN_TIDNO_BEST_EFFORT;
    }
    else
    {
        MAC_SET_CB_IS_QOS_DATA(pst_tx_ctl, OAL_FALSE);
    ...
    if (OAL_PTR_NULL != g_st_dmac_arp_offload_rom_cb.ao_cb_encap)
    {
        g_st_dmac_arp_offload_rom_cb.ao_cb_encap(pst_dmac_vap,pst_dmac_user,pst_tx_buf,us_payload_len,puc_dst_mac_addr);
    }
...
oal_void dmac_ao_encap_arp_response(dmac_vap_stru *pst_dmac_vap,
                                               ...
                                               oal_uint8 *puc_dst_mac_addr)
{
    ...
    oal_set_mac_addr(pst_tx_arp_hdr->auc_ar_sha, mac_mib_get_StationID(pst_mac_vap));
...
oal_bool_enum_uint8 dmac_ao_is_ipv4_addr_owner(dmac_vap_stru *pst_dmac_vap, oal_uint8 *puc_ipv4_addr)
{
    ...
    if (OAL_PTR_NULL !=  g_st_dmac_arp_offload_rom_cb.ao_is_ipv4_addr_cb)
    {
        return g_st_dmac_arp_offload_rom_cb.ao_is_ipv4_addr_cb(pst_dmac_vap, puc_ipv4_addr);
    }
...
dmac_rx_frame_ctrl_enum_uint8 dmac_ao_process_arp_offload(dmac_vap_stru *pst_dmac_vap, dmac_user_stru *pst_dmac_user, oal_netbuf_stru *pst_netbuf, oal_netbuf_stru** ppst_tx_buf)
{
    mac_vap_stru                        *pst_mac_vap   = &pst_dmac_vap->st_vap_base_info;
    ...
    {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_PWR, "{dmac_ao_process_arp_offload::alloc netbuff failed.}");
        return DMAC_RX_FRAME_CTRL_GOON;
    }
                                     ...
                                     puc_dst_mac_addr))
    {
        ...
        g_st_dmac_tx_bss_comm_rom_cb.p_dmac_tx_excp_free_netbuf(pst_tx_buf);
        return DMAC_RX_FRAME_CTRL_GOON;
    }
...
/*lint -save -e416 */
dmac_rx_frame_ctrl_enum_uint8 dmac_ao_encap_na(dmac_vap_stru *pst_dmac_vap,
                                                          dmac_user_stru *pst_dmac_user,
                                                          oal_netbuf_stru *pst_netbuf,
                                                          oal_netbuf_stru **ppst_tx_buf)
{
    mac_vap_stru                        *pst_mac_vap = &pst_dmac_vap->st_vap_base_info;
    ...
    *(oal_uint32 *)pst_tx_ipv6_hdr = OAL_HOST2NET_LONG(0x60000000);
    ...
    if (OAL_FALSE == en_is_dad)
    {
        ...
        pst_tx_nd_hdr->rsv[0] = 1;  //The length of the option (including the type and length fields) in units of 8 octets.

        /*lint +e415 */
        oal_memcopy(&(pst_tx_nd_hdr->rsv[1]), mac_mib_get_StationID(pst_mac_vap), OAL_MAC_ADDR_LEN);

        /* 8.填写IPv6伪首部 */
        pst_tx_ipv6_pseudo_hdr = (oal_ipv6_pseudo_hdr_stru *)(&(pst_tx_nd_hdr->rsv[1]) + OAL_MAC_ADDR_LEN);
    }
    else
    {
        /* 7.DAD没有选项字段 */
...
oal_bool_enum_uint8 dmac_ao_is_ipv6_addr_owner(dmac_vap_stru *pst_dmac_vap, oal_uint8 *puc_ipv6_addr)
{
    oal_uint32  ul_loop;
    oal_uint32  u6_addr32_zero[4]={0x0,0x0,0x0,0x0};

    if (OAL_PTR_NULL != g_st_dmac_arp_offload_rom_cb.ao_is_ipv6_addr_cb)
    {
        return g_st_dmac_arp_offload_rom_cb.ao_is_ipv6_addr_cb(pst_dmac_vap,puc_ipv6_addr);
    }
...
oal_bool_enum_uint8 dmac_ao_is_ipv6_global_ucst_exist(dmac_vap_stru *pst_dmac_vap)
{
    oal_uint32  ul_loop;
...
dmac_rx_frame_ctrl_enum_uint8 dmac_ao_process_nd_offload(dmac_vap_stru *pst_dmac_vap, dmac_user_stru *pst_dmac_user, oal_netbuf_stru *pst_netbuf, oal_netbuf_stru **ppst_tx_buf)
{
    mac_vap_stru                        *pst_mac_vap = &pst_dmac_vap->st_vap_base_info;
    ...
    switch (pst_rx_nd_hdr->icmph.icmp6_type)
    {
        ...
        case MAC_ND_NSOL:
        {
            ...
            if (OAL_TRUE == dmac_ao_is_ipv6_addr_owner(pst_dmac_vap, pst_rx_nd_hdr->target.s6_addr))
            {
                ...
                return dmac_ao_encap_na(pst_dmac_vap, pst_dmac_user, pst_netbuf, ppst_tx_buf);
            }
...
/*lint -save -e415 */
oal_uint8 dmac_ao_get_dhcp_type(oal_uint8 *puc_pos, oal_uint8 *puc_packet_end)
{
    oal_uint8       *puc_opt;
...
dmac_rx_frame_ctrl_enum_uint8 dmac_ao_process_dhcp_filter(dmac_vap_stru *pst_dmac_vap, oal_netbuf_stru *pst_netbuf)
{
    mac_vap_stru                        *pst_mac_vap = &pst_dmac_vap->st_vap_base_info;
    ...
    en_more_frag = (oal_bool_enum_uint8)pst_frame_hdr->st_frame_control.bit_more_frag;
    if (en_more_frag == OAL_TRUE)
    {
        /* 分片帧上报Host侧处理 */
        return DMAC_RX_FRAME_CTRL_GOON;
    }
    ...
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_PWR, "{dmac_ao_process_dhcp_filter::Get DHCP type failed and drop.}");
        return DMAC_RX_FRAME_CTRL_DROP;
    }
    ...
    switch (uc_type)
    {
        ...
        case MAC_DHCP_NAK:
        {
            ...
            if (0 != oal_memcmp(mac_mib_get_StationID(pst_mac_vap), pst_rx_dhcp_hdr->chaddr, WLAN_MAC_ADDR_LEN))
            {
                return DMAC_RX_FRAME_CTRL_DROP;
            }
...
oal_uint32 dmac_ao_get_dhcpv6_client_mac_addr(oal_uint8 *puc_pos, oal_uint8 *puc_packet_end, oal_uint8 *puc_client_mac_addr)
{
    /****************************************************************
...
dmac_rx_frame_ctrl_enum_uint8 dmac_ao_process_dhcpv6_filter(dmac_vap_stru *pst_dmac_vap , oal_netbuf_stru *pst_netbuf)
{
    mac_vap_stru                        *pst_mac_vap = &pst_dmac_vap->st_vap_base_info;
    ...
    if (0 != oal_memcmp(mac_mib_get_StationID(pst_mac_vap), auc_client_mac_addr, WLAN_MAC_ADDR_LEN))
    {
        return DMAC_RX_FRAME_CTRL_DROP;
    }
...
dmac_rx_frame_ctrl_enum_uint8 dmac_ao_sta_process_multicast_filter(dmac_vap_stru * pst_dmac_vap , oal_netbuf_stru *pst_netbuf)
{
    mac_vap_stru                        *pst_mac_vap = &pst_dmac_vap->st_vap_base_info;
    ...
    if (OAL_HOST2NET_SHORT(ETHER_TYPE_IP) == pst_snap->us_ether_type)
    {
        ...
        if (dmac_ao_is_ipv4_broadcast(pst_dmac_vap, pst_ipv4->ul_daddr)
            || OAL_IPV4_IS_PERMANENT_GROUP(pst_ipv4->ul_daddr))
        {
            return DMAC_RX_FRAME_CTRL_DROP;
        }
    }
