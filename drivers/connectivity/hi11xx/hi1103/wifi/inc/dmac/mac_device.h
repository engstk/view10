...
typedef enum
{
    ...
    MAC_M2S_MODE_CONFIGURE    = 1,  /* 切换参数配置模式 */
...
typedef struct
{
    mac_m2s_mode_enum_uint8         en_cfg_m2s_mode;     /* 0:参数查询模式; 1:参数配置模式;2.切换模式;3.软切换测试模式;4.硬切换测试模式 */
    union
    {
        struct
        {
            oal_bool_enum_uint8      en_m2s_type;    /* 切换类型 */
            wlan_nss_enum_uint8      en_m2s_nss;     /*0:切换到siso 1:切换到mimo*/
            oal_uint8                uc_chain;       /* 使用rf通道 1/2/3 分别是通道1的siso 通道2的siso 双通道的mimo */
...
typedef struct
{
    ...
    //oal_hcc_chn_enum_uint8      en_hcc_chn_type;                                /* 当前使用hcc通道类型，0:sdio 1: pcie */
    oal_bool_enum_uint8         en_hcc_chn_switch;                              /* sdio-pcie动态切换开关 */
    oal_uint8                   auc_resv[3];
}mac_chip_stru;
...
extern oal_void mac_blacklist_get_pointer(oal_uint8 uc_dev_id, oal_uint8 uc_chip_id, mac_blacklist_info_stru **pst_blacklist_info);
#endif
