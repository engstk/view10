...
OAL_STATIC oal_uint32  wal_hipriv_set_tx_pow_param(oal_net_device_stru *pst_net_dev, oal_int8 *pc_param)
{
    ...
    /* 命令格式: hipriv "vap0 set_tx_pow sar_lelvel 0/1/2/3" 设置降sar等级               */
...
OAL_STATIC oal_uint32 wal_hipriv_set_pm_debug_switch(oal_net_device_stru *pst_net_dev, oal_int8 *pc_param)
{
    ...
    do
    {
        ...
        {
            OAL_IO_PRINT("CMD format::sh hipriv.sh 'wlan0 pm_debug [srb 0|1] [dto 0|1] [nfi 0|1]'");
...
OAL_STATIC oal_uint32 wal_hipriv_btcoex_preempt_type(oal_net_device_stru *pst_net_dev, oal_int8 *pc_param)
{
    ...
    if((0 == pst_btcoex_preempt_mgr->uc_cfg_preempt_mode && pst_btcoex_preempt_mgr->uc_cfg_preempt_type > 3)||
        (1 == pst_btcoex_preempt_mgr->uc_cfg_preempt_mode && pst_btcoex_preempt_mgr->uc_cfg_preempt_type > 15))
    {
...
oal_uint32  wal_hipriv_set_m2s_switch(oal_net_device_stru *pst_net_dev, oal_int8 *pc_param)
{
    ...
    switch (pst_m2s_mgr->en_cfg_m2s_mode)
    {
        ...
        case MAC_M2S_MODE_HW_TEST:
            ...
            pst_m2s_mgr->pri_data.test_mode.uc_chain = (oal_uint8)oal_atoi(ac_name);
            if(pst_m2s_mgr->pri_data.test_mode.uc_chain > WLAN_RF_CHANNEL_NUMS)
            {
                OAM_WARNING_LOG1(0, OAM_SF_M2S, "{wal_hipriv_set_m2s_switch::pst_m2s_mgr->pri_data.test_mode.uc_chain error [%d] 0/1/2!}\r\n",
                    pst_m2s_mgr->pri_data.test_mode.uc_chain);
                return OAL_ERR_CODE_INVALID_CONFIG;
            }
            ...
            if(2 == pst_m2s_mgr->pri_data.test_mode.uc_chain)
            {
                pst_m2s_mgr->pri_data.test_mode.en_m2s_nss = WLAN_DOUBLE_NSS;
            }
            else
            {
                pst_m2s_mgr->pri_data.test_mode.en_m2s_nss = WLAN_SINGLE_NSS;
            }

            /* 为了兼容测试命令和驱动，phy_chain需要按照1 2 3来执行 */
            pst_m2s_mgr->pri_data.test_mode.uc_chain++;
            break;
...
OAL_STATIC oal_uint32  wal_hipriv_11v_cfg_bsst(oal_net_device_stru *pst_cfg_net_dev, oal_int8 *pc_param)
{
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(0, OAM_SF_BSSTRANSITION, "{wal_hipriv_11v_cfg_bsst::wal_get_cmd_one_arg return err_code %d!}\r\n", ul_ret);
        return ul_ret;
    }
    /* 参数0为关闭能力位 1位打开能力位，其他参数不支持 */
    uc_debug_switch = (oal_uint8)oal_atoi((const oal_int8 *)ac_name);
    ...
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(OAL_SUCC != l_ret))
    {
        OAM_WARNING_LOG1(0, OAM_SF_BSSTRANSITION, "{wal_hipriv_11v_cfg_bsst::return err code %d!}\r\n", l_ret);
        return (oal_uint32)l_ret;
    }
...
OAL_STATIC oal_uint32  wal_hipriv_11v_tx_query(oal_net_device_stru *pst_net_dev, oal_int8 *pc_param)
{
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(0, OAM_SF_BSSTRANSITION, "{wal_hipriv_11v_tx_query::wal_get_cmd_one_arg return err_code [%d]!}\r\n", ul_ret);
        return ul_ret;
    }
    oal_strtoaddr(ac_name, auc_mac_addr);
    /***************************************************************************
    ...
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(OAL_SUCC != l_ret))
    {
        OAM_WARNING_LOG1(0, OAM_SF_BSSTRANSITION, "{wal_hipriv_11v_tx_query::return err code [%d]!}\r\n", l_ret);
        return (oal_uint32)l_ret;
    }
...
{"11v_cfg_bsst",  wal_hipriv_11v_cfg_bsst},                       /* 11v bss transition特性配置 sh hipriv.sh "vap0 11v_cfg_bsst 0|1"
                                                                        0:关闭11V特性；1:打开11V特性*/
#ifdef _PRE_DEBUG_MODE                                              /* Debug模式下增加调试触发接口 */
