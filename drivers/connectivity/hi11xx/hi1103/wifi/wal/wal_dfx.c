...
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)&&(_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    l_plat_ret = plat_exception_handler(0, 0, ul_exception_type);
#endif
...
oal_uint32 wal_dfr_excp_rx(oal_uint8 uc_device_id, oal_uint32 ul_exception_type)
{
    ...
    /* log现在进入异常处理流程 */
    OAM_WARNING_LOG2(0, OAM_SF_DFR, "{wal_dfr_excp_rx:: Enter the exception processing, type[%d] dev_ID[%d].}", ul_exception_type, uc_device_id);
