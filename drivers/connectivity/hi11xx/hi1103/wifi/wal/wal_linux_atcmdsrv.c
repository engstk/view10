...
OAL_STATIC oal_int32  wal_atcmsrv_ioctl_set_txpower(oal_net_device_stru *pst_net_dev, oal_int32 l_txpower)
{
    ...
    *((oal_int32 *)(st_write_msg.auc_value)) = (oal_int32)(l_txpower * 10);
...
OAL_STATIC oal_int32  wal_atcmsrv_ioctl_get_dbb_num(oal_net_device_stru *pst_net_dev, oal_int8 *pc_dbb_num)
{
    ...
    ***************************************************************************/
    ...
    else
    {
        ...
        {
            OAM_WARNING_LOG0(0, OAM_SF_ANY, "wal_atcmsrv_ioctl_get_dbb_num:ul_dbb_num[0x0225020a].");
        }
    #else
        #if 0   //暂时关闭, 待1103 DBB版本号固定后再打开
        /* 1103投片前DBB版本号不确定，暂以B075的FPGA版本号作为判断*/
        if(0x03380c06 != pst_hmac_vap->st_atcmdsrv_get_status.ul_dbb_num)
        {
            OAM_ERROR_LOG1(0, OAM_SF_ANY, "wal_atcmsrv_ioctl_get_dbb_num:ul_dbb_num[0x%x],not match 0x03380c06", pst_hmac_vap->st_atcmdsrv_get_status.ul_dbb_num);
            return -OAL_EINVAL;
        }
        else
        {
            OAM_WARNING_LOG0(0, OAM_SF_ANY, "wal_atcmsrv_ioctl_get_dbb_num:ul_dbb_num[0x03380c06].");
        }
        #endif
    #endif
...
OAL_STATIC oal_int32  wal_atcmsrv_ioctl_lte_gpio_get(oal_int32 l_check_lte_gpio)
{
 ...
 oal_uint64                      ul_gpio_wakeup_host_int_get_save = 0;
...
#if ((_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION) && (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1103_HOST))
    if(g_st_wlan_customize.ul_lte_gpio_check_switch == 1)
    {
        /*获取lte共存管脚结果*/
        l_ret = wal_atcmsrv_ioctl_lte_gpio_check(pst_net_dev);
...
OAL_STATIC oal_int32 wal_atcmsrv_ioctl_get_dp_init(oal_net_device_stru *pst_net_dev, oal_int8 *c_dp_init)
{
    ...
    if (c_dp_init[0] == OAL_TRUE)
    {
        ...
        {
            c_dp_init[uc_i+DP_INIT_FLAG_NUM] = pst_cali_data_c0->ast_2Gcali_param[uc_i].st_cali_tx_power_cmp_2G.c_dp_init;      //core 0 的dp_init
            c_dp_init[uc_i+DP_INIT_FLAG_NUM+DP_INIT_EACH_CORE_NUM] = pst_cali_data_c1->ast_2Gcali_param[uc_i].st_cali_tx_power_cmp_2G.c_dp_init;    // core 1的dp_init
        }
    }
