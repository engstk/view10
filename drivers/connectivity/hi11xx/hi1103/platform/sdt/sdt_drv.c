...
OAL_STATIC OAL_INLINE oal_int32  sdt_drv_report_data2app(oal_netbuf_stru *pst_netbuf, oam_data_type_enum_uint8 en_type, oam_primid_type_enum_uint8 en_prim)
{
    ...
    l_ret = oal_workqueue_schedule(&g_st_sdt_drv_mng_entry.rx_wifi_work);
...
oal_int32  sdt_drv_send_data_to_wifi(oal_uint8  *puc_param, oal_int32  l_len)
{
    ...
    switch(pc_buf[1])
    {
        ...
        case OAM_DATA_TYPE_SAMPLE:
            if (OAL_PTR_NULL != g_st_oam_wal_func_hook.p_wal_recv_sample_data_func)
            {
                l_ret = g_st_oam_wal_func_hook.p_wal_recv_sample_data_func(&puc_data[8], (oal_uint16)i_len);
            }
            break;
#endif
        ...
        case OAM_DATA_TYPE_AUTOCALI:
            if (OAL_PTR_NULL != g_st_oam_wal_func_hook.p_wal_recv_autocali_data_func)
            {
                l_ret = g_st_oam_wal_func_hook.p_wal_recv_autocali_data_func(&puc_data[8], (oal_uint16)i_len);
            }
            break;
#endif
...
oal_void  sdt_drv_netlink_recv(oal_netbuf_stru  *pst_netbuf)
{
    ...
    if (OAL_NETBUF_LEN(pst_netbuf) >= OAL_NLMSG_SPACE(0))
    {
        ...
        if (!OAL_NLMSG_OK(pst_nlhdr, OAL_NETBUF_LEN(pst_netbuf)))
        {
            ...
            OAM_ERROR_LOG2(0, OAM_SF_ANY, "{sdt_drv_netlink_recv::[ERROR]invaild netlink buff data packge data len = :%u,skb_buff data len = %u\n.}",
                                                    pst_nlhdr->nlmsg_len,OAL_NETBUF_LEN(pst_netbuf));
            return;
        }
...
oal_void  sdt_drv_main_exit(oal_void)
{
    ...
    oal_cancel_work_sync(&g_st_sdt_drv_mng_entry.rx_wifi_work);
