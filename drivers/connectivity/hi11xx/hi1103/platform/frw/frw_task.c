...
oal_uint32  frw_task_init(oal_void)
{
    ...
    for (ul_core_id = 0; ul_core_id < WLAN_FRW_MAX_NUM_CORES; ul_core_id++)
    {
        ...
        oal_task_init(&g_ast_event_task[ul_core_id].st_ipi_tasklet, frw_task_ipi_handler, FRW_DST_CORE(ul_core_id));
    }
    return OAL_SUCC;
}
...
oal_void frw_task_exit(oal_void)
{
    ...
    for (ul_core_id = 0; ul_core_id < WLAN_FRW_MAX_NUM_CORES; ul_core_id++)
    {
        ...
        oal_task_kill(&g_ast_event_task[ul_core_id].st_ipi_tasklet);
    }
}
...
OAL_STATIC oal_void frw_task_ipi_handler(oal_uint ui_arg)
{
    ...
    oal_smp_call_function_single(ul_remote_id, frw_remote_task_receive, &g_ast_event_task[ul_remote_id].st_event_tasklet, 0);
}
...
oal_void frw_task_sched(oal_uint32 ul_core_id)
{
    ...
    {
        if (oal_task_is_scheduled(&g_ast_event_task[ul_this_cpu].st_ipi_tasklet))
        {
            return;
        }
        oal_task_sched(&g_ast_event_task[ul_this_cpu].st_ipi_tasklet);
    }
}
