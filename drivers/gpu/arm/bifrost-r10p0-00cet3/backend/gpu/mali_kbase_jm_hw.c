...
void kbase_jm_wait_for_zero_jobs(struct kbase_context *kctx)
{
	...
	if (kbase_prepare_to_reset_gpu(kbdev)) {
		...
		gpu_dsm_report(kbdev, 922002004, "Issueing GPU soft-reset because jobs failed to be killed (within %d ms) as part of context termination (e.g. process exit)\n",
								ZAP_TIMEOUT);
#endif
...
#ifdef CONFIG_HUAWEI_DSM
			gpu_dsm_report(kbdev, 922002004, "Issueing GPU soft-reset after hard stopping due to hardware issue!!!!");
#endif
			kbdev->error_num.soft_reset++;
			kbdev->error_num.ts = hisi_getcurtime();
#ifdef CONFIG_HISI_ENABLE_HPM_DATA_COLLECT
...
#ifdef CONFIG_HUAWEI_DSM
		gpu_dsm_report(kbdev, 922002004, "Resetting GPU (allowing up to %d ms)!!!!", RESET_TIMEOUT);
#endif
		kbdev->error_num.soft_reset++;
		kbdev->error_num.ts = hisi_getcurtime();
	}
