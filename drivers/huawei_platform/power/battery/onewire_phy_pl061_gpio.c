...
/* private delay -- same as kernel delay now -- wait for change */
...
static inline void __hw_const_udelay(unsigned long xloops)
{
	unsigned long loops;
...
static void __hw_udelay(unsigned long usecs)
{
	__hw_const_udelay(usecs * 0x10C7UL); /* 2**32 / 1000000 (rounded up) */
}
...
static unsigned char onewire_reset(void)
{
    ...
    __hw_udelay(ow_trq->reset_init_low_us);
    gpio_set_value_unsafe(HIGH_VOLTAGE);
    gpio_direction_input_unsafe();
    __hw_udelay(ow_trq->reset_slave_response_delay_us);
    presence = gpio_get_value_unsafe();
    __hw_udelay(ow_trq->reset_hold_high_us);
...
{
    gpio_direction_output_unsafe(LOW_VOLTAGE);
    __hw_udelay(ow_trq->read_init_low_us);
    gpio_direction_input_unsafe();
    *value =  (*value) | (gpio_get_value_unsafe() << SHIFT_7);
    __hw_udelay(ow_trq->read_residual_us);
}

static inline void onewire_write_bit(const unsigned char bitval)
{
    gpio_direction_output_unsafe(LOW_VOLTAGE);
    __hw_udelay(ow_trq->write_init_low_us);
    gpio_set_value_unsafe(bitval);
    __hw_udelay(ow_trq->write_hold_us);
    gpio_set_value_unsafe(HIGH_VOLTAGE);
    __hw_udelay(ow_trq->write_residual_us);
}
...
static int onewire_phy_pl061_gpio_driver_probe(struct platform_device *pdev)
{
    ...
    gpio_virtual_base_addr = ioremap(pl061_gpio.gpio_phy_base_addr,pl061_gpio.length);
...
device_initcall_sync(onewire_phy_pl061_gpio_driver_init);
