...
static inline int is_mptcp_sk(const struct sock *sk)
{
	return sk != NULL && sock_flag(sk, SOCK_MPTCP_ONCE_SET);
}
...
static inline bool mptcp_fallback_infinite(struct sock *sk, int flag)
{
	...
	mptcp_debug("%s %#x will fallback - pi %d, src %pI4 dst %pI4 from %pS\n",
